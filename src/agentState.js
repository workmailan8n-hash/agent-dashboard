  ...
  update(agentData, deskSet, dt, tick) {
    ...
    if (working) {
      ...
      if (this.slotIdx === -1 || this.activityDur <= 0) {
        ...
        // Consume queued activity (assigned in Sims mode while agent was busy)
        if (this._queuedActivity) {
          const q = this._queuedActivity;
          this._queuedActivity = null;
          const holder = idleOccupied[q.slotIdx];
          if (
            holder === undefined ||
            holder === this.id ||
            holder === "__queued_" + this.id
          ) {
            this.slotIdx = q.slotIdx;
            idleOccupied[this.slotIdx] = this.id;
            this.activityAnim = q.activityAnim;
            this.activityDur = 20 + Math.random() * 20;
            this.arrived = false;
            this.walkTimer = 0;
          }
        }
        ...
        if (this.slotIdx === -1) {
          ...
          this.arrived = false;
          this.walkTimer = 0;
        } // end if (this.slotIdx === -1) random pick
      }
