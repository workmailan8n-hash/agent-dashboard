...
const sndSpawn  = () => { blip(880,.05,'square',.04); setTimeout(()=>blip(1320,.08,'square',.03),60); };
const sndRemove = () => { blip(440,.05,'sawtooth',.03); setTimeout(()=>blip(220,.12,'sawtooth',.02),60); };
const sndState  = () => blip(660,.04,'square',.02);
...
function drawHeadFront(ctx, cx, cy, pal, blink) {
  ...
  if (pal.demon) {
    ...
    ctx.beginPath(); ctx.moveTo(cx-6,cy-16); ctx.lineTo(cx-9,cy-24); ctx.lineTo(cx-3,cy-16); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+6,cy-16); ctx.lineTo(cx+9,cy-24); ctx.lineTo(cx+3,cy-16); ctx.fill();
  }
  if (pal.cat) {
    // Cat ears
    ctx.fillStyle=pal.hair;
    ctx.beginPath(); ctx.moveTo(cx-6,cy-14); ctx.lineTo(cx-10,cy-22); ctx.lineTo(cx-2,cy-14); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+6,cy-14); ctx.lineTo(cx+10,cy-22); ctx.lineTo(cx+2,cy-14); ctx.fill();
    ctx.fillStyle=pal.skin+'cc';
    ctx.beginPath(); ctx.moveTo(cx-6,cy-15); ctx.lineTo(cx-9,cy-21); ctx.lineTo(cx-3,cy-15); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+6,cy-15); ctx.lineTo(cx+9,cy-21); ctx.lineTo(cx+3,cy-15); ctx.fill();
  }
  if (pal.alien) {
    // Alien antennae
    ctx.strokeStyle=pal.hair; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.moveTo(cx-3,cy-16); ctx.quadraticCurveTo(cx-8,cy-26,cx-6,cy-28); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx+3,cy-16); ctx.quadraticCurveTo(cx+8,cy-26,cx+6,cy-28); ctx.stroke();
    ctx.fillStyle=pal.accent;
    ctx.beginPath(); ctx.arc(cx-6,cy-28,2.5,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx+6,cy-28,2.5,0,Math.PI*2); ctx.fill();
  }
  // голова
  const headW = pal.alien ? 8 : 7;
  ctx.fillStyle=pal.skin;
  ctx.beginPath(); ctx.ellipse(cx,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
  // Robot face plate
  if (pal.robot) {
    ctx.fillStyle=pal.skin+'cc';
    ctx.beginPath(); ctx.ellipse(cx,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
  ...
  if (pal.elf) {
    ctx.fillStyle=pal.skin;
    ctx.beginPath(); ctx.moveTo(cx-8,cy-9); ctx.lineTo(cx-12,cy-14); ctx.lineTo(cx-6,cy-7); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+8,cy-9); ctx.lineTo(cx+12,cy-14); ctx.lineTo(cx+6,cy-7); ctx.fill();
    px(ctx,cx-11,cy-13,2,2,pal.skin+'aa');
    px(ctx,cx+10,cy-13,2,2,pal.skin+'aa');
  } else {
    px(ctx,cx-8,cy-9,3,5,pal.skin); px(ctx,cx+5,cy-9,3,5,pal.skin);
    px(ctx,cx-7,cy-8,2,3,'#d4a07880');
  }
  // волосы
  if (pal.bun) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-17,5,5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-14,7,5,0,-Math.PI,0); ctx.fill();
    ...
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-13,headW,6,0,-Math.PI,0); ctx.fill();
    px(ctx,cx-7,cy-14,4,5,pal.hair); px(ctx,cx+3,cy-14,4,5,pal.hair);
  }
  // глаза
  if (blink) {
    ctx.fillStyle=pal.skin+'dd'; ctx.fillRect(cx-4,cy-10,4,2); ctx.fillRect(cx+1,cy-10,4,2);
  } else if (pal.alien) {
    // Big alien eyes (almond-shaped)
    ctx.fillStyle='#0a1020';
    ctx.beginPath(); ctx.ellipse(cx-4,cy-10,5,3,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+4,cy-10,5,3,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.accent+'bb';
    ctx.beginPath(); ctx.ellipse(cx-4,cy-10,3,2,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+4,cy-10,3,2,0,0,Math.PI*2); ctx.fill();
  } else if (pal.robot) {
    // Visor / LED eyes
    ctx.fillStyle='#001828'; ctx.fillRect(cx-6,cy-12,12,5);
    ctx.fillStyle=pal.accent; ctx.fillRect(cx-5,cy-11,10,3);
    ctx.fillStyle='#ffffff80'; ctx.fillRect(cx-5,cy-11,4,2);
    ctx.fillStyle='#ffffff40'; ctx.fillRect(cx+2,cy-11,3,2);
  } else if (pal.demon) {
    // Glowing eyes
    ctx.fillStyle='#ff2000';
    ctx.beginPath(); ctx.ellipse(cx-3,cy-10,3,2.5,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+3,cy-10,3,2.5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#ff8000aa';
    ctx.beginPath(); ctx.arc(cx-3,cy-10,2,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx+3,cy-10,2,0,Math.PI*2); ctx.fill();
  } else {
    ctx.fillStyle='#1a1b26'; ctx.fillRect(cx-4,cy-11,4,3); ctx.fillRect(cx+1,cy-11,4,3);
    ctx.fillStyle='#ffffff80'; ctx.fillRect(cx-4,cy-11,2,2); ctx.fillRect(cx+1,cy-11,2,2);
    if (pal.fatigue) { ctx.fillStyle='#80405050'; ctx.fillRect(cx-4,cy-8,4,1); ctx.fillRect(cx+1,cy-8,4,1); }
    if (pal.glasses) {
      ctx.strokeStyle='#808090'; ctx.lineWidth=1;
      ctx.strokeRect(cx-5,cy-12,5,4); ctx.strokeRect(cx,cy-12,5,4);
      ctx.beginPath(); ctx.moveTo(cx-5,cy-10); ctx.lineTo(cx-8,cy-9); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(cx+5,cy-10); ctx.lineTo(cx+8,cy-9); ctx.stroke();
    }
  }
  // рот
  if (!pal.robot) {
    if (pal.cat) {
      ctx.fillStyle='#e070a0'; ctx.fillRect(cx-1,cy-5,2,2); // cat nose
      ctx.strokeStyle='#c06080'; ctx.lineWidth=0.8;
      ctx.beginPath(); ctx.moveTo(cx,cy-4); ctx.lineTo(cx-3,cy-2); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(cx,cy-4); ctx.lineTo(cx+3,cy-2); ctx.stroke();
    } else {
      ctx.fillStyle='#c8906090'; ctx.fillRect(cx-3,cy-5,6,2);
    }
  }
  if (pal.beard && !pal.robot && !pal.alien && !pal.demon && !pal.cat) {
    px(ctx,cx-4,cy-4,8,3,pal.hair+'bb'); px(ctx,cx-3,cy-2,6,2,pal.hair+'99');
  }
  if (pal.scarf) { px(ctx,cx-5,cy+1,10,3,pal.accent+'cc'); px(ctx,cx-4,cy+3,3,4,pal.accent); }
}

function drawHeadBack(ctx, cx, cy, pal) {
  px(ctx,cx-2,cy,4,4,pal.skin); // шея
  const headW = pal.alien ? 8 : 7;
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
  ...
  // волосы сзади
  if (pal.bun) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-8,headW+1,9,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-17,5,5,0,0,Math.PI*2); ctx.fill();
  } else if (pal.hat) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-8,7,8,0,0,Math.PI*2); ctx.fill();
    px(ctx,cx-7,cy-21,14,6,pal.accent);
    px(ctx,cx-9,cy-17,18,4,pal.accent+'cc');
  } else if (!pal.robot && !pal.alien) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
    px(ctx,cx-6,cy-2,12,5,pal.hair);
  } else if (pal.robot) {
    ctx.fillStyle='#304050'; ctx.beginPath(); ctx.ellipse(cx,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
  }
}
...
function drawHeadSide(ctx, cx, cy, pal, flipLeft) {
  ...
  if (pal.elf) {
    ctx.fillStyle=pal.skin;
    ctx.beginPath(); ctx.moveTo(cx-d*8,cy-9); ctx.lineTo(cx-d*12,cy-14); ctx.lineTo(cx-d*6,cy-7); ctx.fill();
  } else if (!pal.alien) {
    px(ctx,cx-d*7,cy-9,3,5,pal.skin);
  }
  if (pal.cat) {
    ctx.fillStyle=pal.hair;
    ctx.beginPath(); ctx.moveTo(cx-3*d,cy-14); ctx.lineTo(cx-6*d,cy-22); ctx.lineTo(cx,cy-14); ctx.fill();
    ctx.fillStyle=pal.skin+'cc';
    ctx.beginPath(); ctx.moveTo(cx-3*d,cy-15); ctx.lineTo(cx-5*d,cy-21); ctx.lineTo(cx-1*d,cy-15); ctx.fill();
  }
  if (pal.demon) {
    ctx.fillStyle='#601010';
    ctx.beginPath(); ctx.moveTo(cx+d*2,cy-16); ctx.lineTo(cx+d*5,cy-24); ctx.lineTo(cx-d*1,cy-16); ctx.fill();
  }
  if (pal.bun) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx-d*3,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx-d*2,cy-17,5,5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx+d*2,cy-8,5,7,0,0,Math.PI*2); ctx.fill();
  } else if (pal.hat) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-13,6,5,0,-Math.PI,0); ctx.fill();
    px(ctx,cx-9,cy-18,18,4,pal.accent+'cc');
    px(ctx,cx-7,cy-22,14,6,pal.accent);
  } else if (!pal.robot && !pal.alien) {
    ctx.fillStyle=pal.hair;
    ctx.beginPath(); ctx.ellipse(cx-d*2,cy-10,headW-1,8,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.skin;
    ctx.beginPath(); ctx.ellipse(cx+d*2,cy-7,5,7,0,0,Math.PI*2); ctx.fill();
  } else if (pal.robot) {
    ctx.fillStyle='#304050'; ctx.beginPath(); ctx.ellipse(cx,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx+d*2,cy-8,4,7,0,0,Math.PI*2); ctx.fill();
  }
  if (pal.alien) {
    ctx.fillStyle='#0a1020';
    ctx.beginPath(); ctx.ellipse(cx+d*2,cy-10,4,3,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.accent+'bb';
    ctx.beginPath(); ctx.ellipse(cx+d*2,cy-10,2,2,0,0,Math.PI*2); ctx.fill();
  } else if (pal.robot) {
    ctx.fillStyle='#001828'; ctx.fillRect(cx,cy-12,d*6,5);
    ctx.fillStyle=pal.accent; ctx.fillRect(cx,cy-11,d*5,3);
  } else if (pal.demon) {
    ctx.fillStyle='#ff2000';
    ctx.beginPath(); ctx.ellipse(cx+d*2,cy-10,3,2.5,0,0,Math.PI*2); ctx.fill();
  } else {
    ctx.fillStyle='#1a1b26'; ctx.fillRect(cx+d*1,cy-11,3,3);
    ctx.fillStyle='#ffffff80'; ctx.fillRect(cx+d*1,cy-11,2,2);
    if (pal.glasses) {
      ctx.strokeStyle='#808090'; ctx.lineWidth=1;
  ...
  if (!pal.robot) {
    if (pal.cat) {
      ctx.fillStyle='#e070a0'; ctx.fillRect(cx+d*3,cy-5,2,2);
    } else {
      ctx.fillStyle='#c8906090'; ctx.fillRect(cx+d*2,cy-5,4,2);
    }
  }
  if (pal.beard && !pal.robot && !pal.alien && !pal.demon && !pal.cat) {
    px(ctx,cx+d*1,cy-4,5,3,pal.hair+'bb');
  }
  if (pal.scarf) { px(ctx,cx-4,cy+1,8,3,pal.accent+'cc'); }
}
...
function drawHeadFront34(ctx, cx, cy, pal, flipLeft) {
  ...
  const headW = pal.alien ? 8 : 7;
  ctx.fillStyle=pal.skin;
  ctx.beginPath(); ctx.ellipse(cx+d*1,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
  if (pal.elf) {
    ctx.fillStyle=pal.skin;
    ctx.beginPath(); ctx.moveTo(cx-d*7,cy-9); ctx.lineTo(cx-d*11,cy-14); ctx.lineTo(cx-d*5,cy-7); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+d*8,cy-9); ctx.lineTo(cx+d*11,cy-14); ctx.lineTo(cx+d*6,cy-7); ctx.fill();
  } else if (!pal.alien) {
  ...
  if (pal.cat) {
    ctx.fillStyle=pal.hair;
    ctx.beginPath(); ctx.moveTo(cx-5,cy-14); ctx.lineTo(cx-9,cy-22); ctx.lineTo(cx-1,cy-14); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+5,cy-14); ctx.lineTo(cx+9,cy-22); ctx.lineTo(cx+1,cy-14); ctx.fill();
  }
  if (pal.demon) {
    ctx.fillStyle='#601010';
    ctx.beginPath(); ctx.moveTo(cx-5,cy-16); ctx.lineTo(cx-8,cy-24); ctx.lineTo(cx-2,cy-16); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+5,cy-16); ctx.lineTo(cx+8,cy-24); ctx.lineTo(cx+2,cy-16); ctx.fill();
  }
  if (pal.bun) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-17,5,5,0,0,Math.PI*2); ctx.fill();
    ...
    ctx.fillStyle='#304050'; ctx.beginPath(); ctx.ellipse(cx,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx+d*1,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
  }
  if (pal.alien) {
    ctx.fillStyle='#0a1020';
    ctx.beginPath(); ctx.ellipse(cx+d*3,cy-10,4,3,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx-d*3,cy-10,3,2.5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.accent+'bb';
    ctx.beginPath(); ctx.ellipse(cx+d*3,cy-10,2,2,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx-d*3,cy-10,2,1.5,0,0,Math.PI*2); ctx.fill();
  } else if (pal.robot) {
    ctx.fillStyle='#001828'; ctx.fillRect(cx-5,cy-12,10,5);
    ctx.fillStyle=pal.accent; ctx.fillRect(cx-4,cy-11,8,3);
  } else if (pal.demon) {
    ctx.fillStyle='#ff2000';
    ctx.beginPath(); ctx.ellipse(cx+d*2,cy-10,3,2.5,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx-d*3,cy-10,2,2,0,0,Math.PI*2); ctx.fill();
    ...
    ctx.fillRect(cx+d*1,cy-11,4,3); ctx.fillRect(cx-d*4,cy-11,3,3);
    ctx.fillStyle='#ffffff80';
    ctx.fillRect(cx+d*1,cy-11,2,2); ctx.fillRect(cx-d*4,cy-11,1,2);
    if (pal.glasses) {
      ctx.strokeStyle='#808090'; ctx.lineWidth=1;
      ctx.strokeRect(cx+d*0,cy-12,5,4); ctx.strokeRect(cx-d*5,cy-12,4,4);
    }
  }
  if (!pal.robot) {
    if (pal.cat) {
      ctx.fillStyle='#e070a0'; ctx.fillRect(cx+d*0,cy-5,2,2);
    } else {
      ctx.fillStyle='#c8906090'; ctx.fillRect(cx-2,cy-5,5,2);
    }
  }
  if (pal.beard && !pal.robot && !pal.alien && !pal.demon && !pal.cat) {
    px(ctx,cx-3,cy-4,7,3,pal.hair+'bb');
  }
  if (pal.scarf) { px(ctx,cx-5,cy+1,10,3,pal.accent+'cc'); }
}
...
function drawHeadBack34(ctx, cx, cy, pal, flipLeft) {
  ...
  if (pal.bun) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-8,headW+1,9,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx-d*1,cy-17,5,5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx+d*4,cy-6,3,5,0,0,Math.PI*2); ctx.fill();
  } else if (pal.hat) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-8,7,8,0,0,Math.PI*2); ctx.fill();
    px(ctx,cx-7,cy-21,14,6,pal.accent);
    px(ctx,cx-9,cy-17,18,4,pal.accent+'cc');
  } else if (!pal.robot && !pal.alien) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx-d*1,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx+d*4,cy-6,3,6,0,0,Math.PI*2); ctx.fill();
    px(ctx,cx-6,cy-2,12,5,pal.hair);
  } else if (pal.robot) {
    ctx.fillStyle='#304050'; ctx.beginPath(); ctx.ellipse(cx,cy-8,headW,9,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx+d*3,cy-8,3,6,0,0,Math.PI*2); ctx.fill();
  }
  if (pal.scarf) { px(ctx,cx-5,cy+1,10,3,pal.accent+'cc'); }
}
...
function drawWalkS(ctx, cx, cy, pal, t) {
  ...
  const lL = Math.sin(ph) * 5, lR = -lL;
  const bob = Math.abs(Math.sin(ph)) * 2.5;
  const aL = Math.sin(ph + Math.PI) * 4;
  const aR = Math.sin(ph) * 4;
  shd(ctx, cx, cy+16);
  px(ctx,cx-5,cy+4+lL, 5,9,pal.pants); px(ctx,cx+1,cy+4+lR, 5,9,pal.pants);
  px(ctx,cx-6,cy+12+lL,6,4,'#1a1a2a'); px(ctx,cx-6,cy+14+lL,3,2,'#2a2a3a');
  px(ctx,cx+1,cy+12+lR,6,4,'#1a1a2a'); px(ctx,cx+1,cy+14+lR,3,2,'#2a2a3a');
  px(ctx,cx-6,cy-2-bob,12,9,pal.shirt);
  px(ctx,cx-1,cy-1-bob,2,3,pal.skin);
  px(ctx,cx+2,cy+1-bob,3,4,'#ffffff1a');
  px(ctx,cx-10,cy+aL-bob, 4,7,pal.skin); px(ctx,cx-10,cy+6+aL-bob,4,3,pal.skin);
  px(ctx,cx+6, cy+aR-bob, 4,7,pal.skin); px(ctx,cx+6, cy+6+aR-bob,4,3,pal.skin);
  drawHeadFront(ctx, cx, cy-12-bob, pal, false);
}
function drawWalking(ctx, cx, cy, pal, t) { drawWalkS(ctx, cx, cy, pal, t); }
...
function drawWalkN(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+16);
  px(ctx,cx-5,cy+4+lL, 5,9,pal.pants); px(ctx,cx+1,cy+4+lR, 5,9,pal.pants);
  px(ctx,cx-6,cy+12+lL,6,4,'#1a1a2a'); px(ctx,cx+1,cy+12+lR,6,4,'#1a1a2a');
  px(ctx,cx-6,cy-2-bob,12,9,pal.shirt);
  px(ctx,cx-10,cy+aL-bob, 4,7,pal.shirt); px(ctx,cx-10,cy+6+aL-bob,4,3,pal.skin);
  px(ctx,cx+6, cy+aR-bob, 4,7,pal.shirt); px(ctx,cx+6, cy+6+aR-bob,4,3,pal.skin);
...
function drawWalkE(ctx, cx, cy, pal, t) {
  const ph = t * Math.PI * 2;
  const legF = Math.sin(ph) * 5, legB = -legF;
  const bob = Math.abs(Math.sin(ph)) * 2.5;
  const armF = Math.sin(ph) * 4, armB = -armF;
  shd(ctx, cx, cy+16);
  px(ctx,cx-2,cy+4+legB, 5,9,pal.pants);
  px(ctx,cx-2,cy+12+legB,5,4,'#1a1a2a');
  px(ctx,cx-4,cy-2-bob,9,9,pal.shirt);
  px(ctx,cx-3,cy+armB-bob, 4,7,pal.shirt); px(ctx,cx-3,cy+6+armB-bob,3,3,pal.skin);
  px(ctx,cx-1,cy+4+legF, 5,9,pal.pants);
  px(ctx,cx-1,cy+12+legF,6,4,'#1a1a2a'); px(ctx,cx-1,cy+14+legF,3,2,'#2a2a3a');
  px(ctx,cx+2,cy+armF-bob, 4,7,pal.skin); px(ctx,cx+2,cy+6+armF-bob,3,3,pal.skin);
...
function drawWalkW(ctx, cx, cy, pal, t) {
  ...
  px(ctx,cx-1,cy+armB-bob, 4,7,pal.shirt); px(ctx,cx-1,cy+6+armB-bob,3,3,pal.skin);
  px(ctx,cx-4,cy+4+legF, 5,9,pal.pants);
  px(ctx,cx-5,cy+12+legF,6,4,'#1a1a2a'); px(ctx,cx-5,cy+14+legF,3,2,'#2a2a3a');
  px(ctx,cx-6,cy+armF-bob, 4,7,pal.skin); px(ctx,cx-6,cy+6+armF-bob,3,3,pal.skin);
...
function drawWalkSE(ctx, cx, cy, pal, t) {
  ...
  const aR = Math.sin(ph) * 4;
  shd(ctx, cx, cy+16);
  px(ctx,cx-4,cy+4+lL, 5,9,pal.pants); px(ctx,cx+2,cy+4+lR, 5,9,pal.pants);
  px(ctx,cx-5,cy+12+lL,6,4,'#1a1a2a'); px(ctx,cx+2,cy+12+lR,6,4,'#1a1a2a');
  px(ctx,cx-5,cy-2-bob,11,9,pal.shirt);
  px(ctx,cx+1,cy+1-bob,3,4,'#ffffff1a');
  px(ctx,cx-9,cy+aL-bob, 4,7,pal.skin); px(ctx,cx-9,cy+6+aL-bob,3,3,pal.skin);
  px(ctx,cx+5,cy+aR-bob, 4,7,pal.skin); px(ctx,cx+5,cy+6+aR-bob,3,3,pal.skin);
...
function drawWalkSW(ctx, cx, cy, pal, t) {
  ...
  const aR = Math.sin(ph) * 4;
  shd(ctx, cx, cy+16);
  px(ctx,cx-3,cy+4+lL, 5,9,pal.pants); px(ctx,cx+1,cy+4+lR, 5,9,pal.pants);
  px(ctx,cx-4,cy+12+lL,6,4,'#1a1a2a'); px(ctx,cx+1,cy+12+lR,6,4,'#1a1a2a');
...
function drawWalkNE(ctx, cx, cy, pal, t) {
  ...
  px(ctx,cx-5,cy+12+lL,6,4,'#1a1a2a'); px(ctx,cx+2,cy+12+lR,6,4,'#1a1a2a');
  px(ctx,cx-5,cy-2-bob,11,9,pal.shirt);
  px(ctx,cx-9,cy+aL-bob, 4,7,pal.shirt); px(ctx,cx-9,cy+6+aL-bob,3,3,pal.skin);
  px(ctx,cx+5,cy+aR-bob, 4,7,pal.shirt); px(ctx,cx+5,cy+6+aR-bob,3,3,pal.skin);
...
function drawWalkIdle(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+16);
  px(ctx,cx-5,cy+4, 5,9,pal.pants); px(ctx,cx+1,cy+4, 5,9,pal.pants);
  px(ctx,cx-6,cy+12,6,4,'#1a1a2a'); px(ctx,cx-6,cy+14,3,2,'#2a2a3a');
  px(ctx,cx+1,cy+12,6,4,'#1a1a2a'); px(ctx,cx+1,cy+14,3,2,'#2a2a3a');
  px(ctx,cx-6,cy-2-breathe,12,9,pal.shirt);
  px(ctx,cx-1,cy-1-breathe,2,3,pal.skin);
  px(ctx,cx+2,cy+1-breathe,3,4,'#ffffff1a');
  px(ctx,cx-10,cy-breathe, 4,7,pal.skin); px(ctx,cx-10,cy+6-breathe,4,3,pal.skin);
  px(ctx,cx+6, cy-breathe, 4,7,pal.skin); px(ctx,cx+6, cy+6-breathe,4,3,pal.skin);
  drawHeadFront(ctx, cx, cy-12-breathe, pal, false);
}

// ── directional walk dispatch ────────────────────────────────────
const WALK_DIR_FN = {
  'S': drawWalkS, 'N': drawWalkN, 'E': drawWalkE, 'W': drawWalkW,
  'SE': drawWalkSE, 'SW': drawWalkSW, 'NE': drawWalkNE, 'NW': drawWalkNW,
  'IDLE': drawWalkIdle,
};
...
function drawTypingNormal(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+16);
  // legs
  px(ctx,cx-5,cy+5,5,9,pal.pants); px(ctx,cx+1,cy+5,5,9,pal.pants);
  px(ctx,cx-6,cy+13,6,3,'#1a1a2a'); px(ctx,cx+1,cy+13,6,3,'#1a1a2a');
  // keyboard glow
  ctx.fillStyle='#3a60d840'; ctx.fillRect((cx-12)|0,(cy+2)|0,24,3);
  // body
  px(ctx,cx-6,cy-3,13,9,pal.shirt);
  px(ctx,cx-1,cy-2,2,7,'#ffffff18');
  px(ctx,cx-8,cy-1,3,4,pal.shirt); px(ctx,cx+5,cy-1,3,4,pal.shirt);
  // arms reaching to keyboard
  px(ctx,cx-13,cy+arm,8,3,pal.skin); px(ctx,cx-13,cy+3+arm,5,4,pal.skin);
  px(ctx,cx+5,cy-arm,8,3,pal.skin); px(ctx,cx+5,cy+3-arm,5,4,pal.skin);
...
function drawTypingFurious(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+14);
  px(ctx,cx-4,cy+6,4,8,pal.pants); px(ctx,cx+1,cy+6,4,8,pal.pants);
  // keyboard glow bright
  ctx.fillStyle='#6080ff60'; ctx.fillRect((cx-12)|0,(cy+2)|0,24,3);
  // leaning body
  ...
  ctx.save(); ctx.translate(cx+hshk, cy-12);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-3,8,7,0.2,0,Math.PI*2); ctx.fill();
  px(ctx,-3,-12,3,5,pal.hair); px(ctx,2,-13,4,6,pal.hair); // hair standing up
  ctx.restore();
}
...
function drawThinking(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+14);
  px(ctx,cx-4,cy+7,4,7,pal.pants); px(ctx,cx+1,cy+7,4,7,pal.pants);
  px(ctx,cx-5,cy+13,5,3,'#1a1a2a'); px(ctx,cx+1,cy+13,5,3,'#1a1a2a');
  ...
  ctx.save(); ctx.translate(cx, cy-9+bob);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(-1,0,6,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(-1,-2,7,6,0,-Math.PI,0); ctx.fill();
  if(pal.hat){ctx.fillStyle=pal.accent;ctx.fillRect(-8,-14,16,6);ctx.fillRect(-10,-10,20,4);}
  // eye (one visible)
  const ey = eyeLid < 0.15 ? 0 : 3;
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-5,-3,3,ey);
  ctx.fillStyle='#ffffff60'; ctx.fillRect(-5,-3,1,Math.min(ey,1));
  // thoughtful mouth
  ctx.fillStyle='#c8906060'; ctx.fillRect(-4,3,5,2);
  ctx.fillStyle='#a07050'; ctx.fillRect(-4,5,2,1); // slight frown curve
  if(pal.beard){px(ctx,-5,3,8,3,pal.hair+'bb');}
  ctx.restore();
}
...
function drawDrinkingDesk(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+14);
  px(ctx,cx-4,cy+7,4,7,pal.pants); px(ctx,cx+1,cy+7,4,7,pal.pants);
  px(ctx,cx-6,cy-2,12,9,pal.shirt); px(ctx,cx,cy-1,1,7,'#ffffff18');
  px(ctx,cx-11,cy+1,6,3,pal.skin);
  px(ctx,cx+5,cy-lift*.6,6,3,pal.skin);
  const my = cy-2-lift*.6;
  px(ctx,cx+10,my,7,8,'#c8a060'); px(ctx,cx+10,my,7,2,'#f0f0c0'); px(ctx,cx+17,my+2,3,4,'#a08040');
  ctx.save(); ctx.globalAlpha=0.4+Math.sin(t*Math.PI*4)*.2;
  ctx.fillStyle='#ffffff60'; ctx.beginPath(); ctx.arc(cx+13,my-4,2,0,Math.PI*2); ctx.fill();
  ctx.restore();
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx,cy-9,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-10,7,7,0,0,Math.PI*2); ctx.fill();
...
function drawSpinningChair(ctx, cx, cy, pal, t) {
  ...
  ctx.fillStyle=pal.shirt; ctx.fillRect(-6,-2,12,9);
  ctx.fillStyle=pal.skin; ctx.fillRect(-12,0,6,3); ctx.fillRect(7,0,6,3);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,-9,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-10,7,7,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='#1a1b26';
  ctx.fillRect(-4,-11,3,1); ctx.fillRect(-4,-9,3,1);
  ctx.fillRect(1,-11,3,1);  ctx.fillRect(1,-9,3,1);
  ctx.fillStyle='#c8906080'; ctx.fillRect(-3,-6,5,1);
  ctx.restore();
}
...
function drawCelebrating(ctx, cx, cy, pal, t) {
  ...
  px(ctx,cx-4,jy+8+kick,4,7,pal.pants); px(ctx,cx+1,jy+8-kick,4,7,pal.pants);
  px(ctx,cx-5,jy+14+kick,5,3,'#1a1a2a'); px(ctx,cx+1,jy+14-kick,5,3,'#1a1a2a');
  px(ctx,cx-6,jy,12,9,pal.shirt);
  px(ctx,cx-9,jy-armOff+2,3,8,pal.skin); px(ctx,cx+6,jy-armOff+2,3,8,pal.skin);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx,jy-10,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,jy-13,7,6,0,-Math.PI,0); ctx.fill();
  px(ctx,cx-7,jy-14,4,5,pal.hair); px(ctx,cx+3,jy-14,4,5,pal.hair);
  ctx.fillStyle='#1a1b26'; ctx.fillRect(cx-4,jy-12,3,3); ctx.fillRect(cx+1,jy-12,3,3);
  ctx.fillStyle='#ffffff80'; ctx.fillRect(cx-4,jy-12,1,1); ctx.fillRect(cx+1,jy-12,1,1);
  ctx.fillStyle='#c89060'; ctx.fillRect(cx-4,jy-7,8,2); ctx.fillRect(cx-5,jy-8,2,2); ctx.fillRect(cx+3,jy-8,2,2);
}
...
function drawPhone(ctx, cx, cy, pal, t) {
  ...
  // legs
  px(ctx,cx-20,cy+4,12,6,pal.pants); px(ctx,cx-10,cy+8,4,5,pal.pants);
  ...
  const phx=cx-8, phy=cy-26;
  ctx.fillStyle='#0d0d1e'; ctx.fillRect(phx|0,phy|0,14,20);
  ctx.fillStyle='#1a1a2e'; ctx.fillRect((phx+1)|0,(phy+1)|0,12,18);
  // BRIGHT screen glow
  ctx.save();
  ctx.globalAlpha=glow;
  ctx.fillStyle=SCREEN_C; ctx.fillRect((phx+1)|0,(phy+2)|0,12,14);
  ...
  ctx.beginPath(); ctx.arc(phx+7,phy+18,2,0,Math.PI*2); ctx.fill();
  // thumb
  px(ctx,phx+4,(phy+7+scroll*6)|0,4,5,pal.skin);
  // screen glow on face
  ctx.save(); ctx.globalAlpha=0.18*glow;
  ctx.fillStyle='#5090e0';
  ctx.fillRect((cx-6)|0,(cy-24)|0,14,14);
  ctx.restore();
  // HEAD tilted down looking at phone
  ctx.save(); ctx.translate(cx+1,cy-12); ctx.rotate(0.35);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  px(ctx,6,-2,3,5,pal.skin); // ear
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,8,6,0,-Math.PI,0); ctx.fill();
  if(pal.glasses){ctx.strokeStyle='#808090';ctx.lineWidth=1;ctx.strokeRect(-5,-3,5,4);ctx.strokeRect(0,-3,5,4);}
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-2,4,3); ctx.fillRect(1,-2,4,3);
  ctx.fillStyle='#c8906050'; ctx.fillRect(-2,2,5,2);
  ctx.restore();
}
...
function drawStretching(ctx, cx, cy, pal, t) {
  ...
  px(ctx,cx-8,cy-14,3,5,pal.skin); px(ctx,cx+5,cy-14,3,5,pal.skin); // ears
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-15,7,5,0,-Math.PI,0); ctx.fill();
  if(pal.hat){ctx.fillStyle=pal.accent;ctx.fillRect((cx-8)|0,(cy-26)|0,16,6);ctx.fillRect((cx-10)|0,(cy-22)|0,20,4);}
  if(pal.bun){ctx.fillStyle=pal.hair;ctx.beginPath();ctx.ellipse(cx,cy-22,5,5,0,0,Math.PI*2);ctx.fill();}
  ...
  if(mouthW>0.15){
    ...
    ctx.beginPath(); ctx.ellipse(cx,cy-9,4,5*mouthW,0,0,Math.PI*2); ctx.fill();
    if(mouthW>0.4){
      ctx.fillStyle='#f0e0d0'; ctx.fillRect((cx-3)|0,(cy-11)|0,6,2); // top teeth
      ctx.fillStyle='#e8d0c0'; ctx.fillRect((cx-3)|0,(cy-8+4*mouthW)|0,6,2); // bottom teeth
    }
  } else {
    ctx.fillStyle='#c8906060'; ctx.fillRect((cx-3)|0,(cy-9)|0,6,2);
  }
  if(pal.beard){px(ctx,cx-4,cy-8,8,3,pal.hair+'bb');}
}
...
function drawSleeping(ctx, cx, cy, pal, t) {
  ...
  ctx.fillStyle='#7a6898'; ctx.fillRect((cx-22)|0,(cy-7+br)|0,14,12);
  ctx.fillStyle='#9a82b8'; ctx.fillRect((cx-21)|0,(cy-6+br)|0,12,10);
  ctx.fillStyle='#b090d0'; ctx.fillRect((cx-20)|0,(cy-5+br)|0,6,3); // highlight
  // legs stretched flat
  ...
  ctx.fillStyle=pal.skin;
  ctx.beginPath(); ctx.ellipse(cx+5,cy-1+br,8,7,0,0,Math.PI*2); ctx.fill();
  // ear
  px(ctx,cx+12,cy-2+br,3,5,pal.skin);
  // hair
  ctx.fillStyle=pal.hair;
  ctx.beginPath(); ctx.ellipse(cx+5,cy-4+br,9,6,0,Math.PI,0); ctx.fill();
  if(pal.hat){px(ctx,cx-2,cy-14+br,18,6,pal.accent);px(ctx,cx-4,cy-10+br,22,4,pal.accent+'cc');}
...
function drawDrinkingBeer(ctx, cx, cy, pal, t) {
  ...
  // legs
  px(ctx,cx-22,cy+4,14,6,pal.pants); px(ctx,cx-12,cy+9,5,5,pal.pants);
  ...
  ctx.fillStyle='#e8b820'; ctx.fillRect((cx-14)|0,my|0,13,15); // body
  ctx.fillStyle='#f5d040'; ctx.fillRect((cx-13)|0,my|0,11,5);  // foam top
  ctx.fillStyle='#fffff080'; ctx.fillRect((cx-12)|0,my|0,4,4); // foam hi
  ctx.fillStyle='#c09010'; ctx.fillRect((cx-1)|0,(my+2)|0,5,9); // handle
  ctx.fillStyle='#a07008'; ctx.fillRect((cx-1)|0,(my+2)|0,2,9); // handle dark
  ctx.fillStyle='#c89018'; ctx.fillRect((cx-12)|0,(my+5)|0,9,9); // beer
  ctx.fillStyle='#d4a020'; ctx.fillRect((cx-12)|0,(my+5)|0,2,9); // shine
  // right arm relaxed
  px(ctx,cx+5,cy+1,8,3,pal.skin);
  // head — tilts back when drinking
  ctx.save(); ctx.translate(cx+2,cy-8); ctx.rotate(-tilt*Math.PI/180);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,8,6,0,Math.PI,0); ctx.fill();
  if(!pal.hat){px(ctx,-7,-5,3,4,pal.hair); px(ctx,3,-5,3,4,pal.hair);}
  if(pal.hat){ctx.fillStyle=pal.accent;ctx.fillRect(-8,-14,16,6);ctx.fillRect(-10,-10,20,4);}
  ctx.fillStyle='#1a1b26';
  if(tilt>8){
    // eyes closed, gulping
    ctx.fillRect(-4,-1,4,1); ctx.fillRect(1,-1,4,1);
    ctx.fillStyle='#80302090'; ctx.beginPath(); ctx.ellipse(0,3,4,3,0,0,Math.PI*2); ctx.fill();
  } else {
    ctx.fillRect(-4,-2,4,3); ctx.fillRect(1,-2,4,3);
    ctx.fillStyle='#ffffff60'; ctx.fillRect(-4,-2,1,1); ctx.fillRect(1,-2,1,1);
    ctx.fillStyle='#c8906060'; ctx.fillRect(-2,2,5,2);
  }
  if(pal.beard){ctx.fillStyle=pal.hair+'bb';ctx.fillRect(-4,4,8,3);}
  ctx.restore();
}
...
function drawAtCoffee(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+16);
  px(ctx,cx-4,cy+5,4,9,pal.pants); px(ctx,cx+1,cy+5,4,9,pal.pants);
  px(ctx,cx-5,cy+13,5,3,'#1a1a2a'); px(ctx,cx+1,cy+13,5,3,'#1a1a2a');
  px(ctx,cx-5,cy-3,11,9,pal.shirt);
  // arm reaching to machine
  px(ctx,cx+4,cy-1-press,8,3,pal.skin); px(ctx,cx+11,cy-2-press,3,4,pal.skin);
  // other arm holding cup
  const cupY = cy-1 - Math.max(0,sip)*6;
  px(ctx,cx-11,cy+1,7,3,pal.skin);
  // coffee cup
  ctx.fillStyle='#d0c0a0'; ctx.fillRect((cx-14)|0,(cupY)|0,6,7);
  ctx.fillStyle='#3a1808'; ctx.fillRect((cx-13)|0,(cupY+2)|0,4,5);
  ctx.fillStyle='#ffffff40'; ctx.fillRect((cx-13)|0,cupY|0,2,2);
  // steam from cup
...
function drawAtFridge(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+16);
  px(ctx,cx-4,cy+5+bend*0.3,4,9,pal.pants); px(ctx,cx+1,cy+5+bend*0.3,4,9,pal.pants);
  px(ctx,cx-6,cy+13,6,3,'#1a1a2a'); px(ctx,cx+1,cy+13,6,3,'#1a1a2a');
  ...
  ctx.save(); ctx.translate(cx,cy-10+bend*0.4); ctx.rotate(bend*0.03);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-3,4,3); ctx.fillRect(1,-3,4,3);
  ctx.restore();
}
...
function drawEating(ctx, cx, cy, pal, t) {
  ...
  // sandwich/food
  ctx.fillStyle='#d4b870'; ctx.fillRect((cx-5)|0,(cy-5)|0,10,5);
  ctx.fillStyle='#80c050'; ctx.fillRect((cx-4)|0,(cy-4)|0,8,2);
  ctx.fillStyle='#c05030'; ctx.fillRect((cx-4)|0,(cy-3)|0,8,1);
  ctx.fillStyle='#e0c880'; ctx.fillRect((cx-5)|0,(cy-6)|0,10,2);
  // head
  drawHeadFront(ctx, cx, cy-14, pal, !bite);
  // mouth open/chewing
  if(bite) {
    ctx.fillStyle='#301010';
    ctx.beginPath(); ctx.ellipse(cx,cy-20,3,2,0,0,Math.PI*2); ctx.fill();
  }
}
...
function drawHeadphones(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+16);
  px(ctx,cx-4+hip,cy+5,4,9,pal.pants); px(ctx,cx+1+hip,cy+5,4,9,pal.pants);
  px(ctx,cx-5+hip,cy+13,5,3,'#1a1a2a'); px(ctx,cx+1+hip,cy+13,5,3,'#1a1a2a');
  ...
  drawHeadFront(ctx, cx, cy-12+bob, pal, false);
  // headphone band
  ctx.fillStyle='#404060'; ctx.lineWidth=2;
  ctx.beginPath(); ctx.arc(cx,cy-20+bob,8,Math.PI,0); ctx.stroke();
  ...
  if(Math.random()<0.03) PS.emit(cx+8,cy-22+bob,1,{vx:8,vy:-20,spread:6,gravity:-5,life:1.2,size:4,color:pal.accent});
}
...
function drawAirGuitar(ctx, cx, cy, pal, t) {
  ...
  // head thrown back
  ctx.save(); ctx.translate(cx+lean*0.3, cy-11);
  ctx.rotate(-lean*0.04);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,8,7,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-3,4,3); ctx.fillRect(1,-3,4,3);
  // open mouth (rocking!)
  ctx.fillStyle='#301010'; ctx.fillRect(-3,2,6,3);
  ctx.restore();
  // music note particles
  if(Math.random()<0.06) PS.emit(cx+12,cy-16,1,{vx:(Math.random()-0.5)*30,vy:-40,spread:5,gravity:-10,life:1,size:5,color:pal.accent});
}
...
function drawReading(ctx, cx, cy, pal, t) {
  ...
  // book (open)
  ctx.fillStyle='#8a5030'; ctx.fillRect((cx-8)|0,(cy-8)|0,16,12); // cover
  ctx.fillStyle='#f4f0e8'; ctx.fillRect((cx-7)|0,(cy-7)|0,7,10); // left page
  ctx.fillStyle='#f0ece4'; ctx.fillRect((cx)|0,(cy-7)|0,7,10); // right page
  ctx.fillStyle='#8a807040'; ctx.fillRect((cx-6)|0,(cy-6)|0,5,1);
  ctx.fillRect((cx-6)|0,(cy-4)|0,5,1); ctx.fillRect((cx-6)|0,(cy-2)|0,4,1);
  ctx.fillRect((cx+1)|0,(cy-6)|0,5,1); ctx.fillRect((cx+1)|0,(cy-4)|0,4,1);
  ctx.fillRect((cx+1)|0,(cy-2)|0,5,1);
  // spine
  ctx.fillStyle='#704020'; ctx.fillRect((cx-1)|0,(cy-7)|0,2,10);
  // head looking down at book
  ctx.save(); ctx.translate(cx,cy-12); ctx.rotate(0.15);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,7,6,0,-Math.PI,0); ctx.fill();
  if(pal.glasses){ctx.strokeStyle='#808090';ctx.lineWidth=1;ctx.strokeRect(-5,-3,5,4);ctx.strokeRect(0,-3,5,4);}
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-3,4,3); ctx.fillRect(1,-3,4,3);
...
function drawDeskNap(ctx, cx, cy, pal, t) {
  ...
  ctx.fillStyle=pal.skin;
  ctx.beginPath(); ctx.ellipse(cx+1,cy-8+br,8,6,0.2,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair;
  ctx.beginPath(); ctx.ellipse(cx+1,cy-11+br,8,5,0.2,Math.PI,0); ctx.fill();
  // closed eyes (horizontal lines)
  ctx.fillStyle='#2a1820';
  ctx.fillRect((cx-2)|0,(cy-9+br)|0,4,1);
  ctx.fillRect((cx+4)|0,(cy-9+br)|0,3,1);
  // small smile
  ctx.fillStyle='#c8906050'; ctx.fillRect((cx-1)|0,(cy-6+br)|0,3,1);
  // ZZZ particles
...
function drawDeskYawn(ctx, cx, cy, pal, t) {
  ...
  if (rubT > 0.25) {
    ...
    px(ctx,cx-12,cy-5,8,3,pal.skin); px(ctx,cx-10,cy-8,5,3,pal.skin);
    px(ctx,cx+4,cy-5,8,3,pal.skin);  px(ctx,cx+5,cy-8,5,3,pal.skin);
  } else {
    // stretch — arms raised wide
    const aY = -armRaise|0;
    px(ctx,cx-13,cy-1+aY,7,3,pal.skin); px(ctx,cx-11,cy-4+aY,4,9,pal.skin);
    px(ctx,cx+6,cy-1+aY,7,3,pal.skin);  px(ctx,cx+7,cy-4+aY,4,9,pal.skin);
  }
  // head tilts back a little during yawn
  const headTilt = stretchT * 0.18;
  ctx.save(); ctx.translate(cx, cy-12); ctx.rotate(headTilt);
  // skull
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  // ears
  px(ctx,-8,-3,3,5,pal.skin); px(ctx,5,-3,3,5,pal.skin);
  // hair
  if (pal.bun) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-12,5,5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-8,7,5,0,-Math.PI,0); ctx.fill();
    px(ctx,-7,-9,14,5,pal.hair);
  } else if (pal.hat) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-8,7,5,0,-Math.PI,0); ctx.fill();
    px(ctx,-9,-12,18,4,pal.accent+'cc'); px(ctx,-7,-16,14,6,pal.accent);
  } else if (!pal.robot && !pal.alien) {
    ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-6,7,5,0,-Math.PI,0); ctx.fill();
    px(ctx,-7,-8,4,5,pal.hair); px(ctx,3,-8,4,5,pal.hair);
  }
  // eyes: squint during yawn, rub motion when rubbing
  if (rubT > 0.25) {
    ctx.fillStyle='#2a1820'; // tight squint lines
    ctx.fillRect(-4,-4,4,1); ctx.fillRect(1,-4,4,1);
    ...
    ctx.fillStyle='#1a1b26';
    ctx.fillRect(-4,-4,4,lidH); ctx.fillRect(1,-4,4,lidH);
    if (yawnMouth < 0.5) { ctx.fillStyle='#ffffff80'; ctx.fillRect(-4,-4,2,2); ctx.fillRect(1,-4,2,2); }
    if (pal.fatigue) { ctx.fillStyle='#80405060'; ctx.fillRect(-4,-1,4,1); ctx.fillRect(1,-1,4,1); }
  }
  // mouth: open wide for yawn
  const mW = Math.max(2, yawnMouth * 9)|0;
  const mH = Math.max(1, yawnMouth * 6)|0;
  if (yawnMouth > 0.15) {
    ctx.fillStyle='#601020'; ctx.beginPath(); ctx.ellipse(0,1,mW/2,mH/2,0,0,Math.PI*2); ctx.fill();
    if (mW > 4) { ctx.fillStyle='#ffffff80'; ctx.fillRect(-mW/2+1,0,mW-2,1); } // teeth
  } else {
    ctx.fillStyle='#c8906090'; ctx.fillRect(-3,1,6,2);
  }
  ctx.restore();
  ...
  if (yawnMouth > 0.4 && Math.random() < 0.06) {
    PS.emit(cx + (Math.random()-0.5)*8, cy-24, 1, {
      ...
      spread:2, gravity:-3, life:1.1, size:3+Math.random()*2, color:'#c8d3f5aa',
    });
  }
}
...
function drawDoodling(ctx, cx, cy, pal, t) {
  ...
  // paper on desk
  ctx.fillStyle='#f4f0e8'; ctx.fillRect((cx-8)|0,(cy-5)|0,14,10);
  // scribbles
  ctx.fillStyle='#404060'; ctx.lineWidth=1;
  ctx.beginPath();
  ctx.moveTo(cx-6,cy-3); ctx.lineTo(cx-2,cy-1); ctx.lineTo(cx+2,cy-3);
  ctx.moveTo(cx-5,cy); ctx.lineTo(cx,cy+2);
  ctx.stroke();
  // other arm supporting head
  px(ctx,cx-11,cy+1,7,3,pal.skin);
  // head
  ctx.save(); ctx.translate(cx-2,cy-12); ctx.rotate(0.15);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-3,4,3); ctx.fillRect(1,-3,4,3);
  ctx.restore();
...
function drawPlantCare(ctx, cx, cy, pal, t) {
  ...
  // crouching legs
  px(ctx,cx-7,cy+6,6,7,pal.pants); px(ctx,cx+1,cy+6,6,7,pal.pants);
  px(ctx,cx-9,cy+12,5,3,'#1a1a2a'); px(ctx,cx+3,cy+12,5,3,'#1a1a2a');
  // body crouched
  ...
  ctx.save(); ctx.translate(cx,cy-6); ctx.rotate(0.2);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-3,4,3); ctx.fillRect(1,-3,4,3);
  // smile at plant
  ctx.fillStyle='#c89060'; ctx.fillRect(-3,3,7,2);
  ctx.restore();
}
...
function drawChattingL(ctx, cx, cy, pal, t) {
  ...
  ctx.save(); ctx.translate(cx+2,cy-12+nod);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(-1,0,7,8,0,0,Math.PI*2); ctx.fill();
  px(ctx,5,-2,3,5,pal.skin); // ear facing right
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(-1,-2,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-3,4,3); ctx.fillRect(1,-3,4,3);
  ctx.fillStyle='#c8906060'; ctx.fillRect(-2,3,5,2);
  ctx.restore();
  // speech bubble dots
  if((ph*2|0)%8<4) {
    ctx.save(); ctx.fillStyle='rgba(200,210,255,0.9)';
    rrect(ctx,cx+12,cy-22,20,12,3); ctx.fill();
    ctx.fillStyle='#1a1b26'; ctx.font="5px 'Press Start 2P',monospace";
...
function drawChattingR(ctx, cx, cy, pal, t) {
  ...
  ctx.save(); ctx.translate(cx-2,cy-12+listen);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(1,0,7,8,0,0,Math.PI*2); ctx.fill();
...
function drawArguingL(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+16);
  px(ctx,cx-4+shake,cy+5,4,9,pal.pants); px(ctx,cx+1+shake,cy+5,4,9,pal.pants);
  ...
  // head shaking
  ctx.save(); ctx.translate(cx+2+shake,cy-12);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(-1,0,7,8,0,0,Math.PI*2); ctx.fill();
  px(ctx,5,-2,3,5,pal.skin);
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(-1,-2,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-3,4,3); ctx.fillRect(1,-3,4,3);
  ctx.fillStyle='#c03020'; ctx.fillRect(-3,2,7,3); // open mouth arguing
  ctx.restore();
  // anger symbol
  if((ph*1.5|0)%6<3) {
    ctx.save(); ctx.fillStyle='#ff6040'; ctx.font="8px sans-serif";
    ctx.fillText('!',cx+18,cy-20); ctx.restore();
  }
}
...
function drawArguingR(ctx, cx, cy, pal, t) {
  ...
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(1,-2,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-3,4,3); ctx.fillRect(1,-3,4,3);
  ctx.fillStyle='#c03020'; ctx.fillRect(-3,2,6,3);
  ctx.restore();
  if((ph*1.2|0)%5<2) {
    ctx.save(); ctx.fillStyle='#ff8020'; ctx.font="8px sans-serif";
    ctx.fillText('?!',cx-26,cy-20); ctx.restore();
  }
}
...
function drawGossipingL(ctx, cx, cy, pal, t) {
  ...
  ctx.save(); ctx.translate(cx+2+lean*0.4,cy-12);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(-1,0,7,8,0,0,Math.PI*2); ctx.fill();
  px(ctx,5,-2,3,5,pal.skin);
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(-1,-2,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-5,-3,4,3); ctx.fillRect(0,-3,4,3);
...
function drawGossipingR(ctx, cx, cy, pal, t) {
  ...
  ctx.save(); ctx.translate(cx-2,cy-12-shock*0.2);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(1,0,7,8,0,0,Math.PI*2); ctx.fill();
  px(ctx,-8,-2,3,5,pal.skin);
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(1,-2,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-3,4,3); ctx.fillRect(1,-3,4,3);
  ...
  if(shock>2){
    ctx.fillRect(-4,-4,5,4); ctx.fillRect(1,-4,5,4); // wide eyes
    ctx.fillStyle='#ffffff80'; ctx.fillRect(-3,-4,2,2); ctx.fillRect(2,-4,2,2);
  }
...
function drawGaming(ctx, cx, cy, pal, t) {
  ...
  px(ctx,cx-8,cy+6,7,8,pal.pants); px(ctx,cx+2,cy+6,7,8,pal.pants);
  px(ctx,cx-10,cy+13,7,4,'#1a1a2a'); px(ctx,cx+3,cy+13,7,4,'#1a1a2a');
  // body leaning slightly back
  px(ctx,cx-6,cy-2,13,9,pal.shirt);
  // arms holding controller
  px(ctx,cx-10,cy+2,5,4,pal.skin); px(ctx,cx+6,cy+2,5,4,pal.skin);
  // controller
  const gx=cx-7, gy=cy+3;
  ctx.fillStyle='#1a1a2e'; ctx.fillRect(gx,gy,14,6);
  ctx.fillStyle='#f7768e'; ctx.fillRect(gx+1+btn,gy+1,3,3);
  ctx.fillStyle='#9ece6a'; ctx.fillRect(gx+7,gy+1,3,3);
  // head — eyes wide on TV
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx,cy-9,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-12,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(cx-5,cy-12,4,4); ctx.fillRect(cx+1,cy-12,4,4);
  // TV screen glow on face
  ctx.save(); ctx.globalAlpha=0.15+Math.sin(t*Math.PI*2)*0.05;
  ctx.fillStyle='#5090ff'; ctx.fillRect(cx-8,cy-18,16,12);
  ctx.restore();
}
...
function drawTreadmill(ctx, cx, cy, pal, t) {
  ...
  // legs
  px(ctx,cx-5,jy+5+legL*.3,4,9,pal.pants); px(ctx,cx+2,jy+5+legR*.3,4,9,pal.pants);
  px(ctx,cx-7,jy+14+legL*.5,5,4,'#1a1a2a'); px(ctx,cx+2,jy+14+legR*.5,5,4,'#1a1a2a');
  // body
  px(ctx,cx-6,jy-1,12,9,pal.shirt);
  // arms pumping
  px(ctx,cx-10,jy+armL,4,8,pal.skin); px(ctx,cx+6,jy+armR,4,8,pal.skin);
  // head bobbing
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx,jy-10,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,jy-13,7,6,0,-Math.PI,0); ctx.fill();
  // determined eyes
  ctx.fillStyle='#1a1b26'; ctx.fillRect(cx-5,cy-13,4,3); ctx.fillRect(cx+1,cy-13,4,3);
  // sweat drops
  ctx.fillStyle='#88ccff80';
  ctx.beginPath(); ctx.ellipse(cx+9,cy-8,1.5,2.5,0.3,0,Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(cx+11,cy-5,1,2,0.3,0,Math.PI*2); ctx.fill();
  ctx.restore();
}
...
function drawBenchPress(ctx, cx, cy, pal, t) {
  ...
  // Head
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx-14,cy,6,5,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx-14,cy-2,6,4,0,-Math.PI,0); ctx.fill();
  ...
  ctx.beginPath(); ctx.ellipse(cx-18,armY+1,3,5,0,0,Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(cx+18,armY+1,3,5,0,0,Math.PI*2); ctx.fill();
  // Effort face
  ctx.fillStyle='#1a1b26'; ctx.fillRect(cx-17,cy-1,3,2); ctx.fillRect(cx-13,cy-1,3,2);
  // Sweat
  ctx.fillStyle='#88ccff80';
  ctx.beginPath(); ctx.ellipse(cx-20,cy-2,1,2,0,0,Math.PI*2); ctx.fill();
}
...
function drawBoxing(ctx, cx, cy, pal, t) {
  ...
  ctx.fillStyle='#c04040'; ctx.beginPath(); ctx.arc(lx-2,ly+2,4,0,Math.PI*2); ctx.fill();
  const rx = cx + 6 + rightPunch * 0.3;
  const ry = jy - 4 - rightPunch * 0.1;
  px(ctx,rx,ry,4,4,pal.skin);
  ctx.fillStyle='#c04040'; ctx.beginPath(); ctx.arc(rx+6,ry+2,4,0,Math.PI*2); ctx.fill();
  // Head
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx,jy-10,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,jy-13,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(cx-5,jy-12,4,3); ctx.fillRect(cx+1,jy-12,4,3);
...
function drawLiftingWeights(ctx, cx, cy, pal, t) {
  ...
  // Head
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx,cy-10,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-13,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(cx-5,cy-13,4,3); ctx.fillRect(cx+1,cy-13,4,3);
  // Effort expression
  ctx.fillStyle='#c89060'; ctx.fillRect(cx-3,cy-7,6,2);
  // Sweat
  ctx.fillStyle='#88ccff80';
...
function drawPingPongL(ctx, cx, cy, pal, t) {
  ...
  // hitting arm
  const ax=cx+6+swing*.4, ay=cy-2-swing*.2;
  px(ctx,cx+4,cy+1,6,3,pal.skin);
  ctx.fillStyle='#c04040'; ctx.beginPath(); ctx.ellipse(ax+6,ay-2,7,6,swing*.04,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='#300808'; ctx.beginPath(); ctx.ellipse(ax+6,ay-2,4,3,swing*.04,0,Math.PI*2); ctx.fill();
  // grip line
  ctx.fillStyle='#8b3030'; ctx.fillRect(ax+2,ay,4,2);
  // head
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx,cy-10,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-13,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(cx-5,cy-13,4,3); ctx.fillRect(cx+1,cy-13,4,3);
  ...
  if (ppBall.active) {
    ctx.fillStyle='#1a1b26'; ctx.fillRect(cx-2,cy-11,3,2); // shifted eyes
  }
}
...
function drawPingPongR(ctx, cx, cy, pal, t) {
  ...
  const ax=cx-6-swing*.4, ay=cy-2-swing*.2;
  px(ctx,cx-10,cy+1,6,3,pal.skin);
  ctx.fillStyle='#4040c0'; ctx.beginPath(); ctx.ellipse(ax-6,ay-2,7,6,-swing*.04,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='#080830'; ctx.beginPath(); ctx.ellipse(ax-6,ay-2,4,3,-swing*.04,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='#303080'; ctx.fillRect(ax-8,ay,4,2);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(cx,cy-10,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(cx,cy-13,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(cx-5,cy-13,4,3); ctx.fillRect(cx+1,cy-13,4,3);
  ctx.fillStyle='#c89060'; ctx.fillRect(cx-3,cy-7,6,2);
...
function drawPingPongBall(ctx) {
  ...
  ctx.beginPath(); ctx.ellipse(bx, shadowY + 4, 3 - arc*0.05, 1.5, 0, 0, Math.PI*2); ctx.fill();
  // ball
  ...
  ctx.beginPath(); ctx.arc(bx - speed*1.5, by, 2, 0, Math.PI*2); ctx.fill();
...
function drawAquariumFish(ctx, tick) {
  ...
  const aqx = OX + _aqTx*T, aqy = OY + _aqTy*T;
  const aqW = T*2.5, aqH = T*1.8;
  // Clip to inside of tank glass
  const cx = aqx+4, cy = aqy+4, cw = aqW-8, ch = aqH-8;
  ctx.save();
  ...
  for (const f of aquariumFish) {
    ...
    if (f.x > 0.92) { f.x = 0.92; f.speed = -Math.abs(f.speed); }
    if (f.x < 0.08) { f.x = 0.08; f.speed = Math.abs(f.speed); }
    const fx = cx + f.x * cw;
    const fy = cy + f.y * ch + Math.sin(tick*0.05 + f.phase) * 3;
    const flip = f.speed > 0 ? 1 : -1;
    ctx.save(); ctx.translate(fx, fy); ctx.scale(flip, 1);
    ctx.fillStyle = f.color;
    ctx.beginPath(); ctx.ellipse(0, 0, f.size, f.size*0.6, 0, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.moveTo(-f.size, 0);
    ctx.lineTo(-f.size-3, -2.5); ctx.lineTo(-f.size-3, 2.5); ctx.closePath(); ctx.fill();
    ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(f.size*0.4, -1, 1, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#000'; ctx.beginPath(); ctx.arc(f.size*0.5, -1, 0.5, 0, Math.PI*2); ctx.fill();
    ctx.restore();
  }
  ...
  for (let i=0; i<3; i++) {
    ...
    ctx.beginPath(); ctx.arc(bx, by, 1.5+i*0.5, 0, Math.PI*2); ctx.fill();
  }
...
function drawDustMotes(ctx, tick) {
  ...
  for (const m of dustMotes) {
    ...
    if (m.y > m.wy + m.wh) { m.y = m.wy; m.x = m.wx + Math.random() * (T-12); }
...
function drawFireflies(ctx, tick) {
  const tod = getTimeOfDay();
  if (tod.state !== 'night') { return; }
  if (fireflies.length === 0) {
    for (let w = 0; w < 4; w++) {
      for (let j = 0; j < 2; j++) {
        const wx = OX + (3 + w*6)*T + 4;
        fireflies.push({
          x: wx + Math.random() * (T-12), y: OY + 6 + Math.random() * (T-14),
          vx: (Math.random()-0.5)*0.3, vy: (Math.random()-0.5)*0.3,
          wx: wx, phase: Math.random() * Math.PI * 2,
          trail: []
        });
      }
    }
  ...
  for (const f of fireflies) {
    ...
    for (let t = 0; t < f.trail.length; t++) {
      ...
      ctx.beginPath(); ctx.arc(f.trail[t].x, f.trail[t].y, 1.5, 0, Math.PI*2); ctx.fill();
    }
    // Draw firefly
    ctx.globalAlpha = glow;
...
function drawDartAnimation(ctx, tick) {
  ...
  const dartPlayer = Object.values(agentStates).find(s =>
    s.arrived && s.activityAnim === 'stretching' &&
    IDLE_SPOTS[s.slotIdx]?.type === 'darts'
  );
  if (!dartPlayer) { dartAnims = []; return; }
  ...
  for (const d of dartAnims) {
    ...
    if (d.stuck) {
      // Dart stuck in board
      const dx2 = d.stuckX, dy2 = d.stuckY;
      ctx.save();
      ctx.translate(dx2, dy2);
      ctx.rotate(-Math.PI/2 + d.angle);
      // Dart body
      ctx.fillStyle='#c0c0d0'; ctx.fillRect(-1, -8, 2, 8);
      // Flights (feathers)
      ctx.fillStyle='#cc2020'; ctx.beginPath();
      ctx.moveTo(0, -8); ctx.lineTo(-4, -12); ctx.lineTo(0, -10); ctx.fill();
      ctx.beginPath();
      ctx.moveTo(0, -8); ctx.lineTo(4, -12); ctx.lineTo(0, -10); ctx.fill();
      // Tip
      ctx.fillStyle='#808080'; ctx.fillRect(-0.5, 0, 1, 3);
      ctx.restore();
    } else {
      // Flying dart
      ...
      ctx.rotate(flyAngle);
      // Dart body
      ctx.fillStyle='#c0c0d0'; ctx.fillRect(-1, -6, 2, 6);
      ctx.fillStyle='#cc2020';
      ctx.beginPath(); ctx.moveTo(0,-6); ctx.lineTo(-3,-9); ctx.lineTo(0,-7); ctx.fill();
      ctx.beginPath(); ctx.moveTo(0,-6); ctx.lineTo(3,-9); ctx.lineTo(0,-7); ctx.fill();
      ctx.fillStyle='#808080'; ctx.fillRect(-0.5, 0, 1, 2);
      ctx.restore();
      // Motion trail
...
function drawSmokingBeer(ctx, cx, cy, pal, t) {
  ...
  const cigT = (t % 1);
  const armRaise = cigT < 0.35 ? 0
                 : cigT < 0.65 ? ease.inOut((cigT-0.35)/0.3) * 14
                 : cigT < 0.8  ? ease.inOut((0.8-cigT)/0.15) * 14
                 : 0;
  ...
  const my = cy-12;
  ctx.fillStyle='#e8b820'; ctx.fillRect((cx-14)|0,my|0,13,15);
  ctx.fillStyle='#f5d040'; ctx.fillRect((cx-13)|0,my|0,11,5);
  ctx.fillStyle='#fffff080'; ctx.fillRect((cx-12)|0,my|0,4,4);
  ctx.fillStyle='#c09010'; ctx.fillRect((cx-1)|0,(my+2)|0,5,9);
  ctx.fillStyle='#a07008'; ctx.fillRect((cx-1)|0,(my+2)|0,2,9);
  ctx.fillStyle='#c89018'; ctx.fillRect((cx-12)|0,(my+5)|0,9,9);
  ctx.fillStyle='#d4a020'; ctx.fillRect((cx-12)|0,(my+5)|0,2,9);

  // RIGHT arm — cigarette hand (raises to mouth)
  const cArmY = cy + 1 - armRaise;
  px(ctx, cx+5, cArmY, 8, 3, pal.skin);
  // forearm/wrist
  const cigTipX = cx+13, cigTipY = cArmY - 2;
  // cigarette stick
  ctx.fillStyle = '#f5f0e0';
  ctx.save(); ctx.translate(cigTipX, cigTipY); ctx.rotate(-0.3);
  ctx.fillRect(-1,-1,10,3);
  // filter (orange tip)
  ctx.fillStyle = '#d08030'; ctx.fillRect(8,-1,3,3);
  // lit ember
  ctx.fillStyle = '#ff6020'; ctx.beginPath(); ctx.arc(-1,0.5,2,0,Math.PI*2); ctx.fill();
  ctx.fillStyle = '#ff9040'; ctx.beginPath(); ctx.arc(-1,0.5,1,0,Math.PI*2); ctx.fill();
  ctx.restore();
  ...
  ctx.fillStyle = '#1a1b26';
  ctx.fillRect(-4,-2,4,2); ctx.fillRect(1,-2,4,2);
  ctx.fillStyle = '#ffffff40'; ctx.fillRect(-4,-2,1,1); ctx.fillRect(1,-2,1,1);
  ...
  if (puffing) {
    ...
    ctx.fillRect(-2, 4, 1, 1); ctx.fillRect(2, 4, 1, 1); // tiny smirk
  }
  if (pal.beard) { ctx.fillStyle = pal.hair+'bb'; ctx.fillRect(-4,4,8,3); }
  ctx.restore();
}
...
function drawPettingCat(ctx, cx, cy, pal, t) {
  ...
  ctx.save(); ctx.translate(cx-1,cy-8); ctx.rotate(0.3);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,8,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-2,4,3); ctx.fillRect(1,-2,4,3);
  // happy eyes — squinted
  ctx.fillStyle='#c89060'; ctx.fillRect(-2,3,5,1); // smile
  ctx.restore();
}
...
function drawCleaning(ctx, cx, cy, pal, t) {
  ...
  // head down
  ctx.save(); ctx.translate(cx-2,cy-6); ctx.rotate(0.5);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,7,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,7,5,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-3,-1,3,2); ctx.fillRect(1,-1,3,2);
  // disgusted expression
  ctx.fillStyle='#c8906060'; ctx.fillRect(-2,3,5,1);
...
function drawPlayingArcade(ctx, cx, cy, pal, t) {
  ...
  px(ctx,cx-5,cy+13,5,3,'#1a1a2a'); px(ctx,cx+1,cy+13,5,3,'#1a1a2a');
  ctx.save(); ctx.translate(cx,cy-bounce); ctx.rotate(0.2);
  px(ctx,-6,-2,12,9,pal.shirt);
  ctx.restore();
...
function drawDrinkingEspresso(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+16);
  px(ctx,cx-4,cy+6,4,7,pal.pants); px(ctx,cx+1,cy+6,4,7,pal.pants);
  px(ctx,cx-5,cy+12,5,3,'#1a1a2a'); px(ctx,cx+1,cy+12,5,3,'#1a1a2a');
  ...
  if (!drinking) {
    ...
    ctx.beginPath(); ctx.moveTo(cx-10,cy+cupY-3); ctx.quadraticCurveTo(cx-10+w1,cy+cupY-7,cx-9,cy+cupY-10); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx-8,cy+cupY-3); ctx.quadraticCurveTo(cx-8+w2,cy+cupY-7,cx-7,cy+cupY-10); ctx.stroke();
    ctx.restore();
  }
  px(ctx,cx+5,cy+1,6,3,pal.skin);
...
function drawDJing(ctx, cx, cy, pal, t) {
  ...
  px(ctx,cx-7,cy+13,5,3,'#1a1a2a'); px(ctx,cx+3,cy+13,5,3,'#1a1a2a');
  px(ctx,cx-6,cy-2-bob,12,9,pal.shirt);
  ...
  ctx.save(); ctx.translate(cx,cy-11-bob); ctx.rotate(nod);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-2,4,3); ctx.fillRect(1,-2,4,3);
  ctx.strokeStyle='#303040'; ctx.lineWidth=2;
  ctx.beginPath(); ctx.arc(0,-4,9,Math.PI*1.1,Math.PI*1.9); ctx.stroke();
...
function drawFixingServer(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+16);
  px(ctx,cx-5,cy+4,5,5,pal.pants); px(ctx,cx+1,cy+4,5,5,pal.pants);
  px(ctx,cx-7,cy+8,5,6,pal.pants); px(ctx,cx+3,cy+8,5,6,pal.pants);
  px(ctx,cx-8,cy+13,5,3,'#1a1a2a'); px(ctx,cx+3,cy+13,5,3,'#1a1a2a');
  ...
  ctx.save(); ctx.translate(cx+2,cy-9); ctx.rotate(0.25);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,7,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,7,5,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-1,3,3); ctx.fillRect(2,-1,3,3);
  ctx.restore();
}
...
function drawWatching3DPrint(ctx, cx, cy, pal, t) {
  ...
  ctx.save(); ctx.translate(cx-2,cy-12); ctx.rotate(tilt);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,8,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,7,6,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,0,3,2); ctx.fillRect(2,0,3,2);
  ctx.fillStyle='#c89060'; ctx.fillRect(-2,4,5,1);
  ctx.restore();
}
...
function drawShootingBasket(ctx, cx, cy, pal, t) {
  ...
  if (phase === 0) {
    ...
    ctx.fillStyle='#e88030'; ctx.beginPath(); ctx.arc(cx,cy-14-lift,4,0,Math.PI*2); ctx.fill();
    ctx.strokeStyle='#c06020'; ctx.lineWidth=1;
    ctx.beginPath(); ctx.arc(cx,cy-14-lift,4,0,Math.PI*2); ctx.stroke();
  } else if (phase === 1) {
    ...
    ctx.fillStyle='#e88030'; ctx.beginPath(); ctx.arc(ballX,ballY,4,0,Math.PI*2); ctx.fill();
    ctx.strokeStyle='#c06020'; ctx.lineWidth=1;
    ctx.beginPath(); ctx.arc(ballX,ballY,4,0,Math.PI*2); ctx.stroke();
  } else {
...
function drawPlayingFoosball(ctx, cx, cy, pal, t) {
  ...
  shd(ctx, cx, cy+16);
  px(ctx,cx-6,cy+5,4,9,pal.pants); px(ctx,cx+3,cy+5,4,9,pal.pants);
  px(ctx,cx-7,cy+13,5,3,'#1a1a2a'); px(ctx,cx+3,cy+13,5,3,'#1a1a2a');
  px(ctx,cx-6+lean,cy-2,12,9,pal.shirt);
  px(ctx,cx-10+lean,cy+2,4,3,pal.skin);
  px(ctx,cx+8+lean,cy+2,4,3,pal.skin);
  ctx.strokeStyle='#888'; ctx.lineWidth=2;
  ctx.beginPath(); ctx.moveTo(cx-14+lean,cy+3); ctx.lineTo(cx+14+lean,cy+3); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(cx-14+lean,cy+6); ctx.lineTo(cx+14+lean,cy+6); ctx.stroke();
  ctx.save(); ctx.translate(cx+lean,cy+4); ctx.rotate(twist);
...
function drawUsingTelescope(ctx, cx, cy, pal, t) {
  ...
  px(ctx,cx-9,cy-2,5,3,pal.skin);
  ctx.strokeStyle='#606080'; ctx.lineWidth=3;
  ctx.beginPath(); ctx.moveTo(cx-6,cy-6); ctx.lineTo(cx+12,cy-14+adj*0.5); ctx.stroke();
  ctx.fillStyle='#404060'; ctx.beginPath(); ctx.arc(cx-6,cy-6,3,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='#80a0d0'; ctx.beginPath(); ctx.arc(cx+12,cy-14+adj*0.5,2,0,Math.PI*2); ctx.fill();
  ctx.save(); ctx.translate(cx-4,cy-12); ctx.rotate(0.15);
  ctx.fillStyle=pal.skin; ctx.beginPath(); ctx.ellipse(0,0,7,7,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=pal.hair; ctx.beginPath(); ctx.ellipse(0,-2,7,5,0,-Math.PI,0); ctx.fill();
  ctx.fillStyle='#1a1b26'; ctx.fillRect(-4,-1,3,1);
  ctx.fillRect(2,-1,3,3);
  if (mouthOpen) {
    ctx.fillStyle='#1a1b26'; ctx.beginPath(); ctx.ellipse(0,4,2,2,0,0,Math.PI*2); ctx.fill();
  } else {
    ctx.fillStyle='#c89060'; ctx.fillRect(-2,3,4,1);
  }
  ctx.restore();
}
...
function generateLayout(n) {
  ...
  IDLE_SPOTS.push({ tx:22.8, ty:ACT_ZONE_Y+14.8, anim:'sleeping', type:'nap_pod', w:4,
    _objId:'nap_pod', _defObjTx:22, _defObjTy:ACT_ZONE_Y+14, _offsetX:0.8, _offsetY:0.8 });
...
function drawKanban(ctx) {
  ...
  ctx.save();
  ctx.shadowColor = '#00000060'; ctx.shadowBlur = 10; ctx.shadowOffsetY = 4;
  ...
  for (let i = 0; i < 20; i++) {
    ...
    ctx.beginPath(); ctx.arc(bx + dx, by + dy, 1, 0, Math.PI*2); ctx.fill();
  }
  ...
  // Two columns
  const colW = (bw - 12) / 2;
  const col1x = bx + 5, col2x = bx + 7 + colW;
  const colY = by + 26;
  ...
  // Pushpin decorations
  [[col1x + colW/2, by + 12],[col2x + colW/2, by + 12]].forEach(([px, py2]) => {
    ctx.fillStyle = '#c04040';
    ctx.beginPath(); ctx.arc(px, py2, 3, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = '#ff6060';
    ctx.beginPath(); ctx.arc(px - 1, py2 - 1, 1.2, 0, Math.PI*2); ctx.fill();
  });
...
function drawDynamicWindows(ctx) {
  ...
  const skies = {
    night:  { top:'#0a0a20', bot:'#101830', stars:true, moon:true, shaft:0 },
    dawn:   { top:'#2a1838', bot:'#e06848', stars:false, moon:false, shaft:0.03 },
    day:    { top:'#4a90d0', bot:'#88c8f0', stars:false, moon:false, shaft:0.06 },
    sunset: { top:'#1a1040', bot:'#d06030', stars:false, moon:false, shaft:0.03 },
  };
  const sky = skies[tod.state] || skies.day;

  for (let i = 0; i < 4; i++) {
    const wx = OX + (3 + i*6)*T + 4, wy = OY + 5;
    const ww = T-12, wh = T-14;
    ...
    if (sky.moon && i === 0) {
      ...
      ctx.beginPath(); ctx.arc(wx+ww-4, wy+8, 4, 0, Math.PI*2); ctx.fill();
      ctx.fillStyle = sky.top;
      ctx.beginPath(); ctx.arc(wx+ww-2, wy+7, 3.5, 0, Math.PI*2); ctx.fill(); // crescent
    }
    ...
    if (tod.state === 'day') {
      ...
      ctx.beginPath();
      ctx.arc(cx, wy+8, 3, 0, Math.PI*2); ctx.fill();
      ctx.arc(cx+4, wy+7, 4, 0, Math.PI*2); ctx.fill();
      ctx.arc(cx+8, wy+8, 3, 0, Math.PI*2); ctx.fill();
    }

    // Weather particles outside windows
    { const wx2 = wx + 2, wy2 = wy + 2;
      const weather = getDailyWeather();
      const t = Date.now() * 0.001;
      if (weather === 'rain') {
        ctx.save();
        ctx.beginPath(); ctx.rect(wx2, wy2, ww, wh); ctx.clip();
        ctx.strokeStyle = '#88b8e0'; ctx.lineWidth = 1;
        for (let d = 0; d < 5; d++) {
        ...
        const sunX = wx2 + ww / 2, sunY = wy2;
        for (let r = 0; r < 6; r++) {
          const a = (r / 6) * Math.PI + Math.sin(t * 0.5 + r) * 0.03;
          const grad = ctx.createLinearGradient(sunX, sunY, sunX + Math.cos(a) * wh * 2, sunY + Math.sin(a) * wh * 2);
    ...
    if (sky.shaft > 0) {
      ctx.save(); ctx.globalAlpha = sky.shaft;
      ctx.fillStyle = tod.state==='dawn' ? '#ffcc6040' : tod.state==='sunset' ? '#ff804040' : '#ffe8a040';
      ctx.beginPath();
      ctx.moveTo(wx, wy+T-10); ctx.lineTo(wx+T-8, wy+T-10);
      ctx.lineTo(wx+T, wy+T*2.5); ctx.lineTo(wx-8, wy+T*2.5);
      ctx.closePath(); ctx.fill();
      ctx.restore();
    }
  }
...
function drawOfficeLighting(ctx) {
  ...
  const tints = {
    ...
    dawn:   [255, 140,  60, 0.10], // warm peach — early morning sun creeping in
    day:    [255, 240, 200, 0.00], // no tint — bright neutral daylight
  ...
  if (tod.state === 'dawn') {
    ...
    const night = tints.night, day = tints.day;
...
function drawWallClock(ctx) {
  ...
  const px2 = cx + T/2, py2 = cy + T*0.5;
  const R = 15;
  // Frame
  ctx.fillStyle = '#1a1824';
  ctx.beginPath(); ctx.arc(px2, py2, R+3, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#3a3858';
  ctx.beginPath(); ctx.arc(px2, py2, R+2, 0, Math.PI*2); ctx.fill();
  // Face
  ctx.fillStyle = '#f0ece4';
  ctx.beginPath(); ctx.arc(px2, py2, R, 0, Math.PI*2); ctx.fill();
  // Hour marks
  ctx.strokeStyle = '#1a1824'; ctx.lineWidth = 1.5;
  for (let i = 0; i < 12; i++) {
  ...
  ctx.strokeStyle = '#1a1824'; ctx.lineWidth = 2; ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(px2, py2);
  ...
  ctx.strokeStyle = '#2a2844'; ctx.lineWidth = 1.5;
  ctx.beginPath();
  ctx.moveTo(px2, py2);
...
function drawTrashCan(ctx, level) {
  ...
  const cx2 = tx2 + T/2, cy2 = ty2 + T*0.5;
  // Can body
  ctx.fillStyle = '#404050';
  ctx.beginPath();
...
bgBuf.width=CW; bgBuf.height=900;
const bgx=bgBuf.getContext('2d');
bgx.imageSmoothingEnabled=false;

function ts(tx,ty) { return [OX+tx*T, OY+ty*T]; }
function fillR(ctx,x,y,w,h,c) { if(!c)return; ctx.fillStyle=c; ctx.fillRect(x|0,y|0,w|0,h|0); }

// ── Animated entrance door (drawn over static background each frame) ──────────
function drawEntranceDoor(ctx) {
  const edx = OX + (COLS - 1) * T;
  const edy = OY + (KITCHEN_DOOR_ROW > 0 ? KITCHEN_DOOR_ROW : Math.floor(ROWS * 0.38)) * T;
  const doorH = T * 3;
  ...
  if (doorW > 1) {
    ...
    if (o < 0.55) {
      ...
      ctx.beginPath(); ctx.arc(hx, edy + T * 1.5, 3 * (1 - o * 1.8), 0, Math.PI * 2); ctx.fill();
    }
  }
  ...
  if (o > 0.08) {
    ...
    const lg = ctx.createRadialGradient(edx + T * 0.5, edy + doorH * 0.5, 0, edx + T * 0.5, edy + doorH * 0.5, T * 3);
...
function buildBackground() {
  ...
  for (let row=0; row<ROWS; row++) {
    for (let col=0; col<COLS; col++) {
      ...
      const KIT_ROW_START = 10, KIT_ROW_END = 19, KIT_COL = 23;
      const inKitchen = col > KIT_COL && col < COLS-1 && row >= KIT_ROW_START && row <= KIT_ROW_END;
      const isKitPartWall = col === KIT_COL && row >= KIT_ROW_START && row <= KIT_ROW_END && !(row >= KIT_ROW_START+2 && row <= KIT_ROW_START+5);
      if (wall) {
        if (row===0) {
        ...
        if ((col*7+row*13)%17===0) fillR(ctx,x+(col*5)%24,y,1,T,'rgba(0,0,0,0.04)'); // grain
      }
    }
  }
  // Kitchen partition door frame (fixed position col 23, rows 12-14)
  {
    const dx=OX+23*T, dy=OY+12*T;
  ...
  // ── Main entrance door (right wall) — agents spawn here ──────
  {
    ...
    // Door handle
    ctx.fillStyle='#c0b060';
    ctx.beginPath(); ctx.arc(edx+10, edy+T*1.5, 3, 0, Math.PI*2); ctx.fill();
  }
  ...
  for (let i=0; i<4; i++) {
    ...
    ctx.strokeStyle='#3a3860'; ctx.lineWidth=1.5;
    ctx.strokeRect(wx+.5,wy+.5,T-9,T-11);
    ctx.beginPath();
    ctx.moveTo(wx+(T-8)/2,wy); ctx.lineTo(wx+(T-8)/2,wy+T-10);
    ctx.moveTo(wx,wy+(T-10)/2); ctx.lineTo(wx+T-8,wy+(T-10)/2);
    ctx.strokeStyle='#3a386080'; ctx.stroke();
  }

  // Ceiling strip lights
  for (let li=1; li<COLS-1; li+=6) {
    const lx=OX+li*T+T/2, ly=OY+T;
    ...
    lg.addColorStop(0,'#fffff0'); lg.addColorStop(1,'transparent');
    ctx.fillStyle=lg; ctx.fillRect(OX,ly,CW-OX*2,T*8);
    ctx.restore();
  }
  ...
  if (COUCH_DEFS.length > 0) {
    ...
    ctx.save(); ctx.shadowColor='#00000050'; ctx.shadowBlur=10; ctx.shadowOffsetY=4;
    fillR(ctx,rx,ry,rugW,T*2,'#3a2060');
    ctx.restore();
    fillR(ctx,rx+4,ry+4,rugW-8,T*2-8,'#4a2878');
    ctx.strokeStyle='#5a3898'; ctx.lineWidth=2;
  ...
  for (let _di=0; _di<DESK_DEFS.length; _di++) {
    ...
    const [x,y]=ts(_dATx,_dATy); const W=T*2, H=T;
    ctx.save(); ctx.shadowColor='#00000070'; ctx.shadowBlur=7; ctx.shadowOffsetY=5;
    fillR(ctx,x,y,W,H,DESK_TOP);
    ctx.restore();
    fillR(ctx,x,y,W,3,'#d4c49a');
    fillR(ctx,x,y+H,W,7,DESK_FRONT);
    fillR(ctx,x+W,y,3,H+7,'#7a6030');
    ctx.strokeStyle=DESK_EDGE; ctx.lineWidth=1; ctx.strokeRect(x+.5,y+.5,W-1,H-1);
    ...
    for (let li=0;li<4;li++) fillR(ctx,x+W/2-9,y+8+li*3,6+(li%3)*5,2,gl[li]);
    fillR(ctx,x+W/2-12,y+H-10,24,7,'#1e1e30');
    fillR(ctx,x+W/2-11,y+H-9,22,5,'#2a2a42');
    fillR(ctx,x+W/2+14,y+H-9,6,7,'#2a2a3c');
    const cx2=x+W/2-9, cy2=y+H+9;
    fillR(ctx,cx2,cy2,18,15,CHAIR_C); fillR(ctx,cx2+2,cy2+2,14,11,CHAIR_SEAT);
    fillR(ctx,cx2,cy2-9,18,9,CHAIR_C); fillR(ctx,cx2+2,cy2-8,14,7,CHAIR_SEAT);
    fillR(ctx,cx2-2,cy2+13,4,3,'#181828'); fillR(ctx,cx2+16,cy2+13,4,3,'#181828'); fillR(ctx,cx2+7,cy2+14,4,3,'#181828');
  }
  ...
  for (let _ci=0; _ci<COUCH_DEFS.length; _ci++) {
    ...
    const [x,y]=ts(_cATx,_cATy); const W=w*T, H=T*0.85;
    ctx.save(); ctx.shadowColor='#00000070'; ctx.shadowBlur=9; ctx.shadowOffsetY=5;
    fillR(ctx,x,y,W,H+10,COUCH_C);
    ctx.restore();
    fillR(ctx,x,y,W,10,COUCH_C); fillR(ctx,x+2,y+1,W-4,8,'#4a3880');
    fillR(ctx,x,y+10,W,H,COUCH_SEAT); fillR(ctx,x+3,y+13,W-6,H-6,COUCH_HI);
    fillR(ctx,x,y+10+H,W,6,'#281e50'); fillR(ctx,x+W,y,3,H+16,'#1e1640');
    fillR(ctx,x,y,9,H+10,COUCH_C); fillR(ctx,x+W-9,y,9,H+10,COUCH_C);
    const sec=Math.max(1,Math.floor(w));
    for (let s=1;s<sec;s++) fillR(ctx,x+s*(W/sec)-1,y+10,2,H,'#382860');
    const tx2=x+10, ty2=y+10+H+8;
    fillR(ctx,tx2,ty2,W-20,16,'#5a4830'); fillR(ctx,tx2+2,ty2+2,W-24,12,'#6a5840');
    fillR(ctx,tx2,ty2+16,W-20,4,'#3a2818');
    fillR(ctx,tx2+4,ty2+4,6,6,'#b8a020'); fillR(ctx,tx2+5,ty2+3,4,2,'#f0f0c0');
  }
  ...
  IDLE_SPOTS.filter(s=>s.type==='plant').forEach(({tx:ptx,ty:pty}) => {
    ...
    ctx.beginPath(); ctx.ellipse(pcx, py+T-3, 10, 3, 0, 0, Math.PI*2); ctx.fill();
    // Terracotta pot
    ctx.save(); ctx.shadowColor='#00000060'; ctx.shadowBlur=5;
    ctx.fillStyle='#a04820';
    ctx.beginPath();
    ctx.moveTo(pcx-9,py+T-8); ctx.lineTo(pcx-7,py+T-2); ctx.lineTo(pcx+7,py+T-2); ctx.lineTo(pcx+9,py+T-8); ctx.closePath(); ctx.fill();
    ctx.restore();
    ...
    // Main stem
    ctx.strokeStyle='#2a5018'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(pcx, leafBase); ctx.lineTo(pcx, leafBase-22); ctx.stroke();
    // Left leaf
    ctx.fillStyle='#1e5818';
    ctx.beginPath(); ctx.moveTo(pcx-1, leafBase-10);
    ctx.quadraticCurveTo(pcx-16, leafBase-20, pcx-14, leafBase-28);
    ctx.quadraticCurveTo(pcx-6, leafBase-22, pcx-1, leafBase-10); ctx.fill();
    ctx.fillStyle='#3a7828'; ctx.lineWidth=1;
    ctx.beginPath(); ctx.moveTo(pcx-1, leafBase-10);
    ctx.quadraticCurveTo(pcx-10, leafBase-18, pcx-10, leafBase-26); ctx.stroke();
    // Right leaf
    ctx.fillStyle='#2a6820';
    ctx.beginPath(); ctx.moveTo(pcx+1, leafBase-14);
    ctx.quadraticCurveTo(pcx+16, leafBase-24, pcx+13, leafBase-30);
    ctx.quadraticCurveTo(pcx+4, leafBase-24, pcx+1, leafBase-14); ctx.fill();
    // Center leaf (top)
    ctx.fillStyle='#3a7828';
    ctx.beginPath(); ctx.moveTo(pcx-1, leafBase-20);
    ctx.quadraticCurveTo(pcx-6, leafBase-36, pcx, leafBase-38);
    ctx.quadraticCurveTo(pcx+6, leafBase-36, pcx+1, leafBase-20); ctx.fill();
    // Leaf highlights
    ctx.strokeStyle='#60a840'; ctx.lineWidth=0.8; ctx.globalAlpha=0.7;
    ctx.beginPath(); ctx.moveTo(pcx-1, leafBase-20); ctx.lineTo(pcx, leafBase-38); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(pcx-1, leafBase-10); ctx.lineTo(pcx-10, leafBase-26); ctx.stroke();
    ctx.globalAlpha=1;
  });
  ...
  if (ROWS > 6 && KITCHEN_WALL_COL > 0) {
    ...
    ctx.save(); ctx.shadowColor='#00000070'; ctx.shadowBlur=6; ctx.shadowOffsetY=4;
    fillR(ctx,kx,ky,T*9,T*0.6,'#c4b488'); ctx.restore();
    ...
    const cmx = kx + 6, cmy = ky - 26;
    // Body
    ctx.save(); ctx.shadowColor='#00000060'; ctx.shadowBlur=4;
    fillR(ctx, cmx, cmy+4, 22, 26, '#282838'); ctx.restore();
    fillR(ctx, cmx+1, cmy+4, 20, 2, '#383848'); // top highlight
    // Rounded top
    ctx.fillStyle='#282838';
    ctx.beginPath(); ctx.arc(cmx+11, cmy+5, 10, Math.PI, 0); ctx.fill();
    ctx.fillStyle='#383848';
    ctx.beginPath(); ctx.arc(cmx+11, cmy+5, 9, Math.PI, Math.PI+0.5); ctx.fill();
    // Chrome band
    ...
    // "COFFEE" label
    ctx.fillStyle='#606878'; ctx.font='3px monospace';
    ...
    // ── Microwave ──
    const mcx = kx + T + 8, mcy = ky - 22;
    ctx.save(); ctx.shadowColor='#00000050'; ctx.shadowBlur=3;
    fillR(ctx, mcx, mcy, 30, 22, '#303040'); ctx.restore();
    ...
    // ── Toaster (chrome) ──
    const tsx = kx + T*2 + 14, tsy = ky - 14;
    ctx.save(); ctx.shadowColor='#00000040'; ctx.shadowBlur=2;
    fillR(ctx, tsx, tsy, 16, 14, '#c0c8d0'); ctx.restore();
    ...
    ctx.save(); ctx.shadowColor='#00000060'; ctx.shadowBlur=6;
    fillR(ctx,frx+2,fry,24,T*1.5,'#d0d8e8'); ctx.restore();
    ...
    const tbW = T*4, tbH = T*3;
    ctx.save(); ctx.shadowColor='#00000060'; ctx.shadowBlur=8; ctx.shadowOffsetY=3;
    fillR(ctx,tbx,tby,tbW,tbH,'#a09060'); ctx.restore();
    ...
    // Plates around table
    const plates = [[tbx+T*0.6,tby+T*0.5],[tbx+tbW-T*0.6,tby+T*0.5],[tbx+T*0.6,tby+tbH-T*0.5],[tbx+tbW-T*0.6,tby+tbH-T*0.5]];
    for (const [ppx,ppy] of plates) {
      ctx.fillStyle='#e8e0d0'; ctx.beginPath(); ctx.ellipse(ppx,ppy,7,5,0,0,Math.PI*2); ctx.fill();
      ctx.fillStyle='#d0c8b0'; ctx.beginPath(); ctx.ellipse(ppx,ppy,5,3.5,0,0,Math.PI*2); ctx.fill();
    }
    // Fruit vase in center
    const vx = tbx + tbW/2, vy = tby + tbH/2;
    ctx.fillStyle='#8B4513';
    ctx.beginPath();
    ctx.moveTo(vx-8, vy+4); ctx.lineTo(vx-10, vy-4); ctx.quadraticCurveTo(vx, vy-10, vx+10, vy-4);
    ctx.lineTo(vx+8, vy+4); ctx.quadraticCurveTo(vx, vy+6, vx-8, vy+4);
    ctx.fill();
    ctx.fillStyle='#A0522D';
    ctx.beginPath(); ctx.ellipse(vx, vy-4, 10, 3, 0, 0, Math.PI*2); ctx.fill();
    // Fruits
    ctx.fillStyle='#cc2233';
    ctx.beginPath(); ctx.arc(vx-4, vy-7, 3, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(vx+3, vy-8, 3, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#e8922e';
    ctx.beginPath(); ctx.arc(vx+5, vy-6, 2.5, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(vx-6, vy-6, 2.5, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#6a2d8e';
    for (const gp of [[vx,vy-11],[vx-1.5,vy-10],[vx+1.5,vy-10],[vx,vy-9]]) {
      ctx.beginPath(); ctx.arc(gp[0], gp[1], 1.5, 0, Math.PI*2); ctx.fill();
    }
    // Green leaf on top
    ctx.fillStyle='#3a9040';
    ctx.beginPath(); ctx.ellipse(vx+1, vy-14, 3, 1.5, -0.4, 0, Math.PI*2); ctx.fill();
  ...
  fillR(ctx,npX,npY,10*T,20,'#2a2848'); fillR(ctx,npX+2,npY,10*T-4,18,'#1e1c38');
  ctx.fillStyle='#7aa2f7'; ctx.font="bold 9px 'Press Start 2P',monospace";
  ctx.textAlign='center'; ctx.fillText('AGENT OFFICE',OX+COLS/2*T,npY+14); ctx.textAlign='left';
  ...
  if (ACT_ZONE_Y > 0) {
    ...
    const bW = T * 4.5, bH = T * 3.5;
    // Shadow
    ctx.save(); ctx.shadowColor='#00000090'; ctx.shadowBlur=12; ctx.shadowOffsetY=4;
    fillR(ctx, bsx, bsy, bW, bH, '#3d2410'); ctx.restore();
    // Back panel
    ...
    ctx.fillStyle='#6a4020'; ctx.fillRect(bsx+2, bsy+8, 2, bH-16);
    ctx.fillStyle='#3a2010'; ctx.fillRect(bsx+4, bsy+12, 1, bH-24);
    ctx.fillStyle='#6a4020'; ctx.fillRect(bsx+bW-5, bsy+8, 2, bH-16);
    // Top board
    ...
    const bc1 = ['#f7768e','#7aa2f7','#9ece6a','#e0af68','#bb9af7','#2ac3de','#ff9e64','#f7768e'];
    const bc2 = ['#2ac3de','#e0af68','#bb9af7','#f7768e','#9ece6a','#7aa2f7','#ff9e64','#2ac3de'];
    const bc3 = ['#9ece6a','#ff9e64','#7aa2f7','#f7768e','#e0af68','#bb9af7','#2ac3de','#9ece6a'];
    drawBookRow(bc1, sh[0] - 1, 10);
    drawBookRow(bc2, sh[1] - 1, 10);
    drawBookRow(bc3, sh[2] - 1, 10);
    // Decorative items on top shelf
    // Small plant pot
    ctx.fillStyle='#904020'; ctx.fillRect(bsx+bW-22, bsy+3, 12, 8);
    ctx.fillStyle='#b05030'; ctx.fillRect(bsx+bW-23, bsy+2, 14, 3);
    ctx.fillStyle='#2a6020'; ctx.beginPath(); ctx.arc(bsx+bW-16, bsy-2, 6, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#3a8030'; ctx.beginPath(); ctx.arc(bsx+bW-20, bsy-4, 4, 0, Math.PI*2); ctx.fill();
    // Trophy / award
    ctx.fillStyle='#c8a020'; ctx.fillRect(bsx+14, bsy+4, 8, 6);
    ctx.fillStyle='#e0c040'; ctx.beginPath(); ctx.arc(bsx+18, bsy+2, 5, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#c8a020'; ctx.fillRect(bsx+16, bsy+8, 4, 3);
    ctx.fillStyle='#a08010'; ctx.fillRect(bsx+12, bsy+10, 12, 2);
  }
  ...
  if (KITCHEN_WALL_COL > 0 && KITCHEN_START_ROW > 0) {
    ...
    ctx.save(); ctx.shadowColor='#00000070'; ctx.shadowBlur=6;
    fillR(ctx, prx + 2, pry + 4, T*1.6, T*0.9, '#c0c0cc'); ctx.restore();
    fillR(ctx, prx + 4, pry + 6, T*1.6-4, T*0.9-4, '#d0d0dc');
    // Paper slot (dark groove)
    fillR(ctx, prx + 8, pry + 8, T*1.6-12, 3, '#808090');
    // Status light
    ctx.fillStyle = '#9ece6a';
    ctx.beginPath(); ctx.arc(prx + T*1.6 - 4, pry + 7, 3, 0, Math.PI*2); ctx.fill();
    // Brand strip
  ...
  if (KITCHEN_WALL_COL > 0 && ACT_ZONE_Y > 0) {
    ...
    fillR(ctx, vmx, vmy, T*1.5, T*2.2, '#282840'); ctx.restore();
  ...
  if (ACT_ZONE_Y > 0) {
    ...
    const ctW = T*6, ctH = T*2;
    ctx.save(); ctx.shadowColor='#00000070'; ctx.shadowBlur=10; ctx.shadowOffsetY=4;
    // Oval table
    ctx.fillStyle='#5a3c18';
    ctx.beginPath(); ctx.ellipse(cfx + ctW/2, cfy + ctH/2, ctW/2, ctH/2, 0, 0, Math.PI*2); ctx.fill();
    ctx.restore();
    ctx.fillStyle='#6a4820';
    ctx.beginPath(); ctx.ellipse(cfx + ctW/2, cfy + ctH/2, ctW/2-3, ctH/2-3, 0, 0, Math.PI*2); ctx.fill();
    // Wood grain
    ctx.strokeStyle='#5a3c1840'; ctx.lineWidth=1;
    for (let g=0;g<4;g++){
      ctx.beginPath(); ctx.ellipse(cfx+ctW/2, cfy+ctH/2, ctW/2-6-g*6, ctH/2-4-g*4, 0, 0, Math.PI*2); ctx.stroke();
    }
  ...
  // ════ RIGHT ENTERTAINMENT ZONE ══════════════════════════════════
  {
    ...
    // Cooler body
    ctx.save(); ctx.shadowColor='#00000070'; ctx.shadowBlur=8;
    fillR(ctx, wcx, wcy, T*1.5, T*2, '#d0d8e0'); ctx.restore();
    ...
    ctx.fillStyle='#4488dd'; ctx.fillRect(wcx+6, wcy+T*0.9+4, 7, 4);
    ctx.fillStyle='#dd4444'; ctx.fillRect(wcx+T*1.5-13, wcy+T*0.9+4, 7, 4);
    // Water jug
    ctx.fillStyle='#70b8e880';
    ctx.beginPath(); ctx.ellipse(wcx+T*0.75, wcy-2, 12, 14, 0, Math.PI, 0); ctx.fill();
    ctx.strokeStyle='#5090b8'; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.ellipse(wcx+T*0.75, wcy-2, 12, 14, 0, Math.PI, 0); ctx.stroke();
    // Drip tray
    ...
    // Tank frame
    ctx.save(); ctx.shadowColor='#00000070'; ctx.shadowBlur=6;
    fillR(ctx, aqx, aqy, aqW, aqH, '#1a3a50'); ctx.restore();
    // Glass
    ...
    [6,14,22,30,38,48,56].forEach(px => {
      ctx.beginPath(); ctx.ellipse(aqx+px%aqW+4, aqy+aqH-6, 2+px%3, 1.5, 0, 0, Math.PI*2); ctx.fill();
    });
    // Seaweed
    ctx.strokeStyle='#2a8040'; ctx.lineWidth=2;
    [8, 20, 50].forEach(sx => {
      ctx.beginPath();
    ...
    // Wooden backing
    ctx.save(); ctx.shadowColor='#00000060'; ctx.shadowBlur=4;
    fillR(ctx, dartWallX-DR-3, dartWallY-DR-3, DR*2+6, DR*2+6, '#4a3018'); ctx.restore();
    // Board circle
    ctx.fillStyle='#101010';
    ctx.beginPath(); ctx.arc(dartWallX, dartWallY, DR, 0, Math.PI*2); ctx.fill();
    // Rings
    const rings = [[DR-1,'#1a6030'],[DR-3,'#cc2020'],[DR-5,'#e8d870'],[DR-7,'#1a6030'],[DR-9,'#cc2020'],[4,'#e8d870'],[2,'#cc2020']];
    for (const [r,c] of rings) { ctx.fillStyle=c; ctx.beginPath(); ctx.arc(dartWallX,dartWallY,r,0,Math.PI*2); ctx.fill(); }
    // Wires
    ctx.strokeStyle='#c0c0c040'; ctx.lineWidth=0.5;
    ctx.beginPath(); ctx.moveTo(dartWallX-DR,dartWallY); ctx.lineTo(dartWallX+DR,dartWallY); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(dartWallX,dartWallY-DR); ctx.lineTo(dartWallX,dartWallY+DR); ctx.stroke();
    // Bullseye
    ctx.fillStyle='#f0e8d0'; ctx.beginPath(); ctx.arc(dartWallX,dartWallY,1.5,0,Math.PI*2); ctx.fill();
  ...
  if (ACT_ZONE_Y > 0) {
    ...
    ctx.fillStyle='#ffcc00'; ctx.font="5px 'Press Start 2P',monospace";
    ctx.textAlign='center'; ctx.fillText('ARCADE', arcx+arcW/2, arcy+10); ctx.textAlign='left';
    const marqueeCols = ['#ff0','#0ff','#f0f','#0f0','#ff0','#0ff'];
    for (let li=0; li<6; li++) {
      ctx.fillStyle=marqueeCols[li]; ctx.beginPath();
      ctx.arc(arcx+6+li*9, arcy+1, 2, 0, Math.PI*2); ctx.fill();
    }
    ...
    ctx.fillStyle='#c04040'; ctx.beginPath();
    ctx.arc(arcx+12, arcy+arcH*0.65+2, 4, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#e04040'; ctx.beginPath(); ctx.arc(arcx+arcW-16, arcy+arcH*0.65+6, 3, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#40a0e0'; ctx.beginPath(); ctx.arc(arcx+arcW-8, arcy+arcH*0.65+6, 3, 0, Math.PI*2); ctx.fill();
    ...
    const espW = T*3, espH = T*1.5;
    ctx.save(); ctx.shadowColor='#00000060'; ctx.shadowBlur=6;
    fillR(ctx, espx, espy, espW, espH, '#5a3c20'); ctx.restore();
    ...
    [espx+6, espx+16, espx+espW-18].forEach(cx2 => {
      ...
      ctx.fillStyle='#c0b8a8'; ctx.fillRect(cx2+1, espy-3, 4, 3); ctx.fillStyle='#e0d8c8';
    });
    ...
    const srvW = T*2, srvH = T*2.2;
    ctx.save(); ctx.shadowColor='#00000080'; ctx.shadowBlur=8;
    fillR(ctx, srvx, srvy, srvW, srvH, '#101018'); ctx.restore();
    ...
    for (let ru=0; ru<5; ru++) {
      ...
      for (let li=0; li<4; li++) {
        ...
        ctx.beginPath(); ctx.arc(srvx+8+li*8, ruy+6, 1.5, 0, Math.PI*2); ctx.fill();
      }
    }
    ...
    for (let ci=0; ci<3; ci++) {
      ...
      ctx.quadraticCurveTo(srvx+8+ci*10+5, srvy+srvH+4, srvx+12+ci*10, srvy+srvH-2);
      ctx.stroke();
    }
    ...
    const p3W = T*2, p3H = T*1.8;
    ctx.save(); ctx.shadowColor='#00000060'; ctx.shadowBlur=5;
    fillR(ctx, p3x, p3y, p3W, p3H, '#303040'); ctx.restore();
    ...
    ctx.strokeStyle='#e06020'; ctx.lineWidth=2.5;
    ctx.beginPath(); ctx.ellipse(bbx+T*0.9, bby+T*1.0+2, 8, 4, 0, 0, Math.PI*2); ctx.stroke();
    ctx.strokeStyle='#ffffff80'; ctx.lineWidth=0.8;
    ...
    const fbW = T*3, fbH = T*1.5;
    ctx.save(); ctx.shadowColor='#00000070'; ctx.shadowBlur=6;
    fillR(ctx, fbx, fby, fbW, fbH, '#1a5030'); ctx.restore();
    ...
    for (let ri=0; ri<4; ri++) {
      ...
      ctx.beginPath(); ctx.moveTo(fbx-4, ry); ctx.lineTo(fbx+fbW+4, ry); ctx.stroke();
    ...
    const djW = T*2.5, djH = T*1.2;
    ctx.save(); ctx.shadowColor='#00000060'; ctx.shadowBlur=5;
    fillR(ctx, djx, djy, djW, djH, '#1a1a2a'); ctx.restore();
    fillR(ctx, djx+2, djy+2, djW-4, djH-4, '#222234');
    ctx.fillStyle='#303040'; ctx.beginPath();
    ctx.arc(djx+djW*0.25, djy+djH*0.5, 12, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#1a1a28'; ctx.beginPath();
    ctx.arc(djx+djW*0.25, djy+djH*0.5, 10, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#404050'; ctx.beginPath();
    ctx.arc(djx+djW*0.25, djy+djH*0.5, 3, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#303040'; ctx.beginPath();
    ctx.arc(djx+djW*0.75, djy+djH*0.5, 12, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#1a1a28'; ctx.beginPath();
    ctx.arc(djx+djW*0.75, djy+djH*0.5, 10, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle='#404050'; ctx.beginPath();
    ctx.arc(djx+djW*0.75, djy+djH*0.5, 3, 0, Math.PI*2); ctx.fill();
    ...
    ctx.beginPath(); ctx.arc(djx+djW*0.43, djy+8, 2, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(djx+djW*0.53, djy+10, 2, 0, Math.PI*2); ctx.fill();
    ctx.strokeStyle='#606070'; ctx.lineWidth=2;
    ctx.beginPath();
    ctx.arc(djx-4, djy+djH*0.3, 6, -Math.PI*0.5, Math.PI*0.5); ctx.stroke();
    ctx.fillStyle='#404050';
    ...
    ctx.strokeStyle='#606070'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(telx+T*0.5, tely+T*0.6); ctx.lineTo(telx+2, tely+T*1.6); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(telx+T*0.5, tely+T*0.6); ctx.lineTo(telx+T-2, tely+T*1.6); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(telx+T*0.5, tely+T*0.6); ctx.lineTo(telx+T*0.5+8, tely+T*1.5); ctx.stroke();
    ctx.save();
    ...
    ctx.beginPath(); ctx.arc(telx+T*0.5-12, tely+T*0.5+10, 3, 0, Math.PI*2); ctx.fill();
    ...
    const napW = T*2.5, napH = T*1.2;
    // Pod shell (oval/capsule shape)
    ctx.save(); ctx.shadowColor='#2060c060'; ctx.shadowBlur=10;
    ctx.fillStyle='#1a2840';
    ctx.beginPath();
    ctx.ellipse(napx + napW/2, napy + napH/2 + 4, napW/2 + 2, napH/2 + 4, 0, 0, Math.PI*2);
    ...
    ctx.beginPath();
    ctx.ellipse(napx + napW/2, napy + napH/2, napW/2 - 2, napH/2 - 2, 0, -Math.PI*0.8, -Math.PI*0.1);
    ctx.fill();
    // Visor window (blue-tinted oval)
    ctx.fillStyle='#0a1828';
    ctx.beginPath();
    ctx.ellipse(napx + napW/2, napy + napH/2 + 2, napW/2 - 6, napH/2 - 3, 0, 0, Math.PI*2);
    ctx.fill();
    ...
    // Label
    ctx.fillStyle='#2ac3de'; ctx.font='5px monospace'; ctx.textAlign='center';
  ...
  if (ACT_ZONE_Y > 0) {
    ...
    const doorStart = Math.floor(zoneW * 0.35), doorEnd = Math.floor(zoneW * 0.65);
    ...
    fillR(ctx,t1x+2,t1y+8,T*1.2,T*0.7,'#282840'); ctx.restore();
    ...
    fillR(ctx,drx+6,dry+6,4,4,'#404050'); fillR(ctx,drx+4,dry+7,2,2,'#404050'); fillR(ctx,drx+12,dry+7,2,2,'#404050');
    // Dumbbell 2 (medium, chrome)
    fillR(ctx,drx+20,dry+5,6,4,'#606070'); fillR(ctx,drx+17,dry+6,3,3,'#606070'); fillR(ctx,drx+26,dry+6,3,3,'#606070');
    // Dumbbell 3 (small, dark)
    fillR(ctx,drx+36,dry+6,4,4,'#353545'); fillR(ctx,drx+34,dry+7,2,2,'#353545'); fillR(ctx,drx+42,dry+7,2,2,'#353545');
    // Bottom shelf dumbbells (3)
    const bsy = dry+Math.floor(T*0.4)+4;
    // Dumbbell 4 (big, iron)
    fillR(ctx,drx+5,bsy+2,8,4,'#404050'); fillR(ctx,drx+3,bsy+1,3,5,'#505060'); fillR(ctx,drx+13,bsy+1,3,5,'#505060');
    // Dumbbell 5 (big, chrome)
    fillR(ctx,drx+22,bsy+2,8,4,'#606070'); fillR(ctx,drx+20,bsy+1,3,5,'#707080'); fillR(ctx,drx+30,bsy+1,3,5,'#707080');
    // Dumbbell 6 (medium, dark)
    fillR(ctx,drx+38,bsy+2,6,4,'#353545'); fillR(ctx,drx+36,bsy+1,2,5,'#454555'); fillR(ctx,drx+44,bsy+1,2,5,'#454555');
    ...
    // Weight plates (left)
    ctx.fillStyle='#404050'; ctx.beginPath(); ctx.ellipse(bpx+3,bpy+1,4,6,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#353545'; ctx.beginPath(); ctx.ellipse(bpx+7,bpy+1,3,5,0,0,Math.PI*2); ctx.fill();
    // Weight plates (right)
    ctx.fillStyle='#404050'; ctx.beginPath(); ctx.ellipse(bpx+T*1.8-3,bpy+1,4,6,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#353545'; ctx.beginPath(); ctx.ellipse(bpx+T*1.8-7,bpy+1,3,5,0,0,Math.PI*2); ctx.fill();

    // ── Punching Bag (right of treadmills) ──
    const [pbx,pby] = ts(_gymTx+6, _gymTy+0.5);
    // Chain from ceiling
    ctx.strokeStyle='#808090'; ctx.lineWidth=1;
    ctx.beginPath(); ctx.moveTo(pbx+10,pby-16); ctx.lineTo(pbx+10,pby); ctx.stroke();
    // Mounting bracket
    fillR(ctx,pbx+6,pby-16,8,3,'#505060');
    // Bag body
    ctx.save(); ctx.shadowColor='#00000060'; ctx.shadowBlur=6;
    ctx.fillStyle='#a03020';
    ctx.beginPath(); ctx.ellipse(pbx+10,pby+16,8,18,0,0,Math.PI*2); ctx.fill();
    ctx.restore();
    // Bag highlight
    ctx.fillStyle='#b8402e';
    ctx.beginPath(); ctx.ellipse(pbx+8,pby+12,3,10,0,0,Math.PI*2); ctx.fill();
    // Bag bottom cap
    fillR(ctx,pbx+6,pby+32,8,3,'#802818');
    // Shadow below
    ctx.fillStyle='#00000030';
    ctx.beginPath(); ctx.ellipse(pbx+10,pby+40,10,3,0,0,Math.PI*2); ctx.fill();
    ...
    ctx.beginPath(); ctx.arc(ebx+8,eby+T*0.8-2,6,0,Math.PI*2); ctx.stroke();
    // Pedals
    ...
    ctx.save(); ctx.shadowColor='#00000040'; ctx.shadowBlur=4;
    // Mat
    ...
    ctx.save(); ctx.shadowColor='#0050ff30'; ctx.shadowBlur=20;
    fillR(ctx,tvx,tvy,T*4,T*2.5,'#0a0a18'); ctx.restore();
    ...
    // score
    ctx.fillStyle='#9ece6a'; ctx.font="4px 'Press Start 2P',monospace";
    ...
    ctx.fillStyle='#9ece6a80'; ctx.beginPath(); ctx.arc(cx2+T*1.2-8,cy2+7,2,0,Math.PI*2); ctx.fill();
    ...
    const sfH = T*0.7+10;
    ctx.save(); ctx.shadowColor='#00000070'; ctx.shadowBlur=8;
    fillR(ctx,sfx,sfy,T*4,sfH,COUCH_C); ctx.restore();
    ...
    const ppW = T*6, ppH = T*1.5;
    ctx.save(); ctx.shadowColor='#00000080'; ctx.shadowBlur=10; ctx.shadowOffsetY=6;
    fillR(ctx,ppx,ppy,ppW,ppH,'#164028'); ctx.restore();
    // Surface with border
    ...
    ctx.fillStyle='#ffffffd0'; ctx.fillRect(ppx+ppW/2-2, ppy+ppH-3, 4, 3);
    ...
    const drawPaddle = (ex, ey, color) => {
      ctx.fillStyle='#2a1808'; ctx.beginPath(); ctx.ellipse(ex,ey,5,7,0.3,0,Math.PI*2); ctx.fill();
      ctx.fillStyle=color; ctx.beginPath(); ctx.ellipse(ex,ey,4,6,0.3,0,Math.PI*2); ctx.fill();
      ctx.fillStyle=color+'88'; ctx.beginPath(); ctx.ellipse(ex-1,ey-1,2,3,0.3,0,Math.PI*2); ctx.fill();
    };
  ...
  for (const bowl of CAT_BOWLS) {
    ...
    // Shadow
    ctx.fillStyle='rgba(0,0,0,0.25)';
    ctx.beginPath(); ctx.ellipse(bx+T/2, by+T*0.75+3, 9, 3, 0, 0, Math.PI*2); ctx.fill();
    // Bowl body
    ctx.fillStyle=rimOuter;
    ctx.beginPath(); ctx.arc(bx+T/2, by+T*0.65, 9, 0, Math.PI); ctx.closePath(); ctx.fill();
    ctx.fillStyle=rimInner;
    ctx.beginPath(); ctx.ellipse(bx+T/2, by+T*0.55, 9, 5, 0, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle=rimFloor;
    ctx.beginPath(); ctx.ellipse(bx+T/2, by+T*0.55, 7, 3.5, 0, 0, Math.PI*2); ctx.fill();
    // Label icon
  ...
  for (const bowl of CAT_BOWLS) {
    ...
    if (bowl.type === 'food') {
      if (bowl.hasFod) {
        ...
        const kibble = [[-5,-1],[0,-2],[5,-1],[-2.5,2],[2.5,2]];
        for (const [kx,ky] of kibble) {
          ctx.fillStyle='#c06010';
          ctx.beginPath(); ctx.ellipse(cx+kx, cy+ky, 3, 2.2, kx*0.3, 0, Math.PI*2); ctx.fill();
          ctx.fillStyle='#e8902a';
          ctx.beginPath(); ctx.ellipse(cx+kx-0.5, cy+ky-0.5, 2, 1.5, kx*0.3, 0, Math.PI*2); ctx.fill();
        }
      } else {
        ...
        // "empty" text hint
        ctx.fillStyle='#ffffff50'; ctx.font='5px sans-serif'; ctx.textAlign='center';
        ctx.fillText('…', cx, cy+1); ctx.textAlign='left';
      }
    } else {
      // Water bowl
      if (bowl.hasFod) {
        ...
        ctx.strokeStyle='#60c0ff90'; ctx.lineWidth=1;
        for (let r=0; r<2; r++) {
          const rp = phase + r * Math.PI;
          const rw = 3 + Math.sin(rp) * 1.5;
          ctx.beginPath(); ctx.ellipse(cx, cy, rw, rw*0.4, 0, 0, Math.PI*2); ctx.stroke();
        }
        // Highlight glint
        ctx.fillStyle='#ffffff70';
        ctx.beginPath(); ctx.ellipse(cx-2, cy-1, 2, 1, -0.3, 0, Math.PI); ctx.fill();
      } else {
        // Empty water — dry bowl
        ctx.fillStyle='rgba(40,100,180,0.2)';
        ctx.beginPath(); ctx.ellipse(cx, cy, 5, 2.5, 0, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle='#ffffff40'; ctx.font='5px sans-serif'; ctx.textAlign='center';
        ctx.fillText('…', cx, cy+1); ctx.textAlign='left';
      }
    }
  }
...
function drawDynamicEffects(ctx, tick) {
  ...
  if (KITCHEN_WALL_COL > 0 && KITCHEN_START_ROW > 0) {
    const [_prBlinkTx, _prBlinkTy] = getAdminPos('printer', KITCHEN_WALL_COL-3, KITCHEN_START_ROW-2);
    const [prBx, prBy] = ts(_prBlinkTx, _prBlinkTy);
    const ledX = prBx + T*1.6 - 4, ledY = prBy + 7;
    ...
    if (blink) {
      ...
      prGlow.addColorStop(0, '#9ece6a'); prGlow.addColorStop(1, 'transparent');
      ctx.fillStyle = prGlow; ctx.fillRect(ledX-8, ledY-8, 16, 16);
    } else {
      ctx.globalAlpha = 0.3;
  ...
  if (ACT_ZONE_Y > 0) {
    ...
    const napG = ctx.createRadialGradient(npGx+T, npGy+T*0.6, 0, npGx+T, npGy+T*0.6, T*1.2);
    napG.addColorStop(0, '#40a0ff'); napG.addColorStop(1, 'transparent');
    ctx.fillStyle = napG;
...
function drawCat(ctx, cx, cy, tick, state, catObj) {
  ...
  if (state === 'sleeping') {
    ...
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx,cy+4,13,8,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#d07820';
    // tail curled over body
    ctx.beginPath();
    ctx.arc(cx+6,cy+2,8,Math.PI*0.2,Math.PI*1.1); ctx.lineWidth=4;
    ctx.strokeStyle='#e8902a'; ctx.stroke();
    ctx.arc(cx+6,cy+2,8,Math.PI*0.2,Math.PI*1.1); ctx.lineWidth=2;
    ctx.strokeStyle='#d07820'; ctx.stroke();
    // head tucked, tiny ears
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx-8,cy,7,6,0.4,0,Math.PI*2); ctx.fill();
    // ears (small nubs)
    ctx.beginPath(); ctx.moveTo(cx-13,cy-4); ctx.lineTo(cx-11,cy-9); ctx.lineTo(cx-8,cy-4); ctx.closePath();
    ctx.fillStyle='#e8902a'; ctx.fill();
    // closed eyes
    ctx.strokeStyle='#2a1808'; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.arc(cx-9,cy-1,2,Math.PI,0); ctx.stroke();
    ctx.beginPath(); ctx.arc(cx-5,cy-1,2,Math.PI,0); ctx.stroke();
    // stripes
    ctx.strokeStyle='#c07018'; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.moveTo(cx-2,cy+1); ctx.lineTo(cx+4,cy+3); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx,cy-3); ctx.lineTo(cx+6,cy-1); ctx.stroke();
    return;
  }
  ...
  if (state === 'eating') {
    ...
    ctx.fillStyle='#e8902a';
    // Body leaning forward
    ctx.beginPath(); ctx.ellipse(cx,cy+3,9,7,-0.3,0,Math.PI*2); ctx.fill();
    // Belly
    ctx.fillStyle='#f5c870';
    ctx.beginPath(); ctx.ellipse(cx,cy+5,5,4.5,-0.3,0,Math.PI*2); ctx.fill();
    // Tail up and happy (curled)
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=4;
    ctx.beginPath(); ctx.moveTo(cx-6,cy+2); ctx.quadraticCurveTo(cx-16,cy-4,cx-12,cy-14); ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(cx-6,cy+2); ctx.quadraticCurveTo(cx-16,cy-4,cx-12,cy-14); ctx.stroke();
    // Head down toward bowl (tilted forward)
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx+7,cy+4,7,6,0.5,0,Math.PI*2); ctx.fill();
    // Ears
    ctx.beginPath(); ctx.moveTo(cx+3,cy-1); ctx.lineTo(cx+4,cy-7); ctx.lineTo(cx+8,cy-1); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+8,cy-1); ctx.lineTo(cx+10,cy-7); ctx.lineTo(cx+13,cy-1); ctx.closePath(); ctx.fill();
    // Closed happy eyes (eating)
    ctx.strokeStyle='#2a1808'; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.arc(cx+6,cy+3,2,Math.PI,0); ctx.stroke();
    ctx.beginPath(); ctx.arc(cx+10,cy+3,2,Math.PI,0); ctx.stroke();
    // Nose
    ctx.fillStyle='#e06080';
    ctx.beginPath(); ctx.ellipse(cx+8,cy+6,1.5,1,0,0,Math.PI*2); ctx.fill();
    // Eating particles (little kibble bits)
    const ep = tick * 0.06;
    ctx.fillStyle='#e8902a80';
    ctx.beginPath(); ctx.arc(cx+10+Math.cos(ep)*3, cy+8+Math.sin(ep*1.3)*2, 1.5, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx+7+Math.cos(ep+2)*2, cy+9+Math.sin(ep*0.9)*1.5, 1, 0, Math.PI*2); ctx.fill();
    return;
  }
  ...
  if (state === 'pooping') {
    ...
    // ears
    ctx.beginPath(); ctx.moveTo(cx-5,cy-8); ctx.lineTo(cx-3,cy-15); ctx.lineTo(cx,cy-8); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+1,cy-8); ctx.lineTo(cx+3,cy-15); ctx.lineTo(cx+6,cy-8); ctx.closePath(); ctx.fill();
    // head raised (looking away embarrassed)
    ctx.beginPath(); ctx.ellipse(cx+8,cy-6,7,6,0,0,Math.PI*2); ctx.fill();
    // embarrassed eyes (looking away)
    ctx.fillStyle='#1a1020'; ctx.fillRect((cx+5)|0,(cy-8)|0,2,3); ctx.fillRect((cx+10)|0,(cy-8)|0,2,3);
    ctx.fillStyle='#e06080'; ctx.beginPath(); ctx.ellipse(cx+8,cy-5,2,1.5,0,0,Math.PI*2); ctx.fill(); // nose
    // blush
    ctx.fillStyle='#ff808060';
    ctx.beginPath(); ctx.ellipse(cx+4,cy-5,3,2,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+12,cy-5,3,2,0,0,Math.PI*2); ctx.fill();
    // tail up
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=4;
    ctx.beginPath(); ctx.moveTo(cx-8,cy); ctx.quadraticCurveTo(cx-14,cy-8,cx-10,cy-18); ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(cx-8,cy); ctx.quadraticCurveTo(cx-14,cy-8,cx-10,cy-18); ctx.stroke();
    // squatted legs
    ctx.fillStyle='#d07820';
    ctx.beginPath(); ctx.ellipse(cx-6,cy+8,4,3,0.5,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+4,cy+8,4,3,-0.5,0,Math.PI*2); ctx.fill();
    return;
  }
  ...
  if (state === 'playing' && catObj) {
    ...
    // Body
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx,cy+2,9,8,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#f5c870';
    ctx.beginPath(); ctx.ellipse(cx,cy+4,5,5,0,0,Math.PI*2); ctx.fill();
    // Tail arcing out (target of chase)
    const tAngle = catObj.playAngle * 0.7 + 1;
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=4;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+4);
    ctx.quadraticCurveTo(cx-18,cy-2,cx-12+Math.cos(tAngle)*8,cy-10+Math.sin(tAngle)*6);
    ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+4);
    ctx.quadraticCurveTo(cx-18,cy-2,cx-12+Math.cos(tAngle)*8,cy-10+Math.sin(tAngle)*6);
    ...
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx+4,cy-8,8,7,0,0,Math.PI*2); ctx.fill();
    // Ears
    ctx.beginPath(); ctx.moveTo(cx-1,cy-13); ctx.lineTo(cx+1,cy-20); ctx.lineTo(cx+5,cy-13); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+5,cy-13); ctx.lineTo(cx+8,cy-20); ctx.lineTo(cx+12,cy-13); ctx.closePath(); ctx.fill();
    // Excited eyes (big pupils)
    ctx.fillStyle='#1a1020';
    ctx.beginPath(); ctx.ellipse(cx+3,cy-10,3,3.5,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+9,cy-10,3,3.5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#ffffff';
    ctx.fillRect((cx+2)|0,(cy-12)|0,1,1); ctx.fillRect((cx+8)|0,(cy-12)|0,1,1);
    // Nose
    ctx.fillStyle='#e06080';
    ctx.beginPath(); ctx.ellipse(cx+6,cy-7,2,1.5,0,0,Math.PI*2); ctx.fill();
    ctx.restore();
    return;
  }
  ...
  if (state === 'grooming' && catObj) {
    ...
    // Tail resting
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=4;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+4); ctx.quadraticCurveTo(cx-16,cy+10,cx-14,cy+2); ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+4); ctx.quadraticCurveTo(cx-16,cy+10,cx-14,cy+2); ctx.stroke();
    // Head
    ctx.fillStyle='#e8902a';
    ...
    ctx.fillStyle='#d07820';
    ctx.beginPath(); ctx.ellipse(pawX, pawY, 4, 3, 0.3, 0, Math.PI*2); ctx.fill();
    // Closed content eyes
    ctx.strokeStyle='#2a1808'; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.arc(cx+3,cy-10,2,0,Math.PI); ctx.stroke();
    ctx.beginPath(); ctx.arc(cx+9,cy-10,2,0,Math.PI); ctx.stroke();
    // Nose
    ctx.fillStyle='#e06080';
    ctx.beginPath(); ctx.ellipse(cx+6,cy-7,2,1.5,0,0,Math.PI*2); ctx.fill();
    // Tiny tongue
    if (Math.sin(catObj.groomPhase * 2) > 0.3) {
      ...
      ctx.beginPath(); ctx.ellipse(cx+7,cy-5.5,1.5,1,0,0,Math.PI*2); ctx.fill();
    }
    return;
  }
  ...
  if (state === 'stretching') {
    // Elongated body
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx,cy+3,14,6,-0.15,0,Math.PI*2); ctx.fill();
    // Butt raised
    ctx.beginPath(); ctx.ellipse(cx-8,cy-2,7,6,0.3,0,Math.PI*2); ctx.fill();
    // Front paws stretched forward
    ctx.fillStyle='#d07820';
    ctx.beginPath(); ctx.ellipse(cx+12,cy+8,5,2,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+10,cy+9,5,2,-0.2,0,Math.PI*2); ctx.fill();
    // Head down between paws
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx+10,cy+2,7,6,0.2,0,Math.PI*2); ctx.fill();
    // Ears
    ctx.beginPath(); ctx.moveTo(cx+6,cy-2); ctx.lineTo(cx+7,cy-9); ctx.lineTo(cx+10,cy-2); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+10,cy-2); ctx.lineTo(cx+12,cy-9); ctx.lineTo(cx+15,cy-2); ctx.closePath(); ctx.fill();
    // Closed content eyes
    ctx.strokeStyle='#2a1808'; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.arc(cx+9,cy+1,2,0,Math.PI); ctx.stroke();
    ctx.beginPath(); ctx.arc(cx+13,cy+1,2,0,Math.PI); ctx.stroke();
    // Tail up from butt
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=4;
    ctx.beginPath(); ctx.moveTo(cx-12,cy-2); ctx.quadraticCurveTo(cx-18,cy-10,cx-14,cy-18); ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(cx-12,cy-2); ctx.quadraticCurveTo(cx-18,cy-10,cx-14,cy-18); ctx.stroke();
    return;
  }
  ...
  if (state === 'hunting' && catObj) {
    ...
    // Low crouching body
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx+wiggle*0.3,cy+5,12,5,0,0,Math.PI*2); ctx.fill();
    // Butt wiggling
    ctx.beginPath(); ctx.ellipse(cx-7+wiggle,cy+3,6,5,0,0,Math.PI*2); ctx.fill();
    // Head low, focused
    ctx.beginPath(); ctx.ellipse(cx+9,cy+2,7,6,0.1,0,Math.PI*2); ctx.fill();
    // Ears perked
    ctx.beginPath(); ctx.moveTo(cx+5,cy-2); ctx.lineTo(cx+6,cy-10); ctx.lineTo(cx+9,cy-2); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+9,cy-2); ctx.lineTo(cx+11,cy-10); ctx.lineTo(cx+14,cy-2); ctx.closePath(); ctx.fill();
    // Big focused eyes
    ctx.fillStyle='#1a1020';
    ctx.beginPath(); ctx.ellipse(cx+8,cy,3,4,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+13,cy,3,4,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#3a2860';
    ctx.beginPath(); ctx.ellipse(cx+8,cy,1,3,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+13,cy,1,3,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#ffffff';
    ctx.fillRect((cx+7)|0,(cy-2)|0,1,1); ctx.fillRect((cx+12)|0,(cy-2)|0,1,1);
    // Tail low, twitching
    const tTwitch = Math.sin(tick*0.2)*4;
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=4;
    ctx.beginPath(); ctx.moveTo(cx-10,cy+3); ctx.quadraticCurveTo(cx-18,cy+tTwitch,cx-16,cy-2+tTwitch); ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(cx-10,cy+3); ctx.quadraticCurveTo(cx-18,cy+tTwitch,cx-16,cy-2+tTwitch); ctx.stroke();
    return;
  }
  ...
  if (state === 'knocking' && catObj) {
    ...
    ctx.fillStyle='#1a1020';
    ctx.beginPath(); ctx.ellipse(cx+3,cy-9,2.5,2,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+9,cy-9,2.5,2,0,0,Math.PI*2); ctx.fill();
    // Paw reaching out to push (if in push phase)
    if (catObj.knockPhase >= 1) {
      const pawExt = catObj.knockPhase === 2 ? 6 : Math.sin(tick*0.15)*3;
      ctx.fillStyle='#d07820';
      ctx.beginPath(); ctx.ellipse(cx+12+pawExt,cy+2,4,3,0,0,Math.PI*2); ctx.fill();
    }
    // Nose
    ctx.fillStyle='#e06080';
    ctx.beginPath(); ctx.ellipse(cx+6,cy-7,2,1.5,0,0,Math.PI*2); ctx.fill();
    // Tail
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=4;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+4); ctx.quadraticCurveTo(cx-14,cy-4,cx-10,cy-12); ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+4); ctx.quadraticCurveTo(cx-14,cy-4,cx-10,cy-12); ctx.stroke();
    return;
  }
  ...
  if (state === 'zoomies') {
    ...
    ctx.globalAlpha = 0.3;
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx-4,cy+2,9,8,0,0,Math.PI*2); ctx.fill();
    ctx.globalAlpha = 0.15;
    ctx.beginPath(); ctx.ellipse(cx-8,cy+2,9,8,0,0,Math.PI*2); ctx.fill();
    ctx.globalAlpha = 1.0;
    // Main body
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx,cy+2,9,7,-0.2,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#f5c870';
    ctx.beginPath(); ctx.ellipse(cx,cy+4,5,4,0,0,Math.PI*2); ctx.fill();
    // Head (leaning forward)
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx+7,cy-6,7,6,0.2,0,Math.PI*2); ctx.fill();
    // Ears flattened back
    ctx.beginPath(); ctx.moveTo(cx+1,cy-8); ctx.lineTo(cx-2,cy-14); ctx.lineTo(cx+4,cy-9); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+5,cy-9); ctx.lineTo(cx+2,cy-15); ctx.lineTo(cx+8,cy-10); ctx.closePath(); ctx.fill();
    // Wild eyes
    ctx.fillStyle='#1a1020';
    ctx.beginPath(); ctx.ellipse(cx+6,cy-8,3,3.5,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+11,cy-8,3,3.5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#ffffff';
    ctx.fillRect((cx+5)|0,(cy-10)|0,2,2); ctx.fillRect((cx+10)|0,(cy-10)|0,2,2);
    // Legs in running motion
    const run = tick*0.15;
    ctx.fillStyle='#d07820';
    ctx.beginPath(); ctx.ellipse(cx-3+Math.sin(run)*4,cy+9,4,2,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+5+Math.sin(run+Math.PI)*4,cy+9,4,2,0,0,Math.PI*2); ctx.fill();
    // Tail straight back
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=4;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+2); ctx.lineTo(cx-20,cy-2); ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+2); ctx.lineTo(cx-20,cy-2); ctx.stroke();
    return;
  }
  ...
  if (state === 'purring') {
    ...
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx+pVib,cy+2,9,8,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#f5c870';
    ctx.beginPath(); ctx.ellipse(cx+pVib,cy+4,5,5,0,0,Math.PI*2); ctx.fill();
    // Tail curled around
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=4;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+4); ctx.quadraticCurveTo(cx-14,cy+12,cx-4,cy+10); ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+4); ctx.quadraticCurveTo(cx-14,cy+12,cx-4,cy+10); ctx.stroke();
    // Head
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx+4+pVib,cy-8,8,7,0,0,Math.PI*2); ctx.fill();
    // Ears
    ctx.beginPath(); ctx.moveTo(cx-1,cy-13); ctx.lineTo(cx+1,cy-20); ctx.lineTo(cx+5,cy-13); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+5,cy-13); ctx.lineTo(cx+8,cy-20); ctx.lineTo(cx+12,cy-13); ctx.closePath(); ctx.fill();
    // Happy closed eyes (upturned)
    ctx.strokeStyle='#2a1808'; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.arc(cx+3,cy-10,2.5,Math.PI,0); ctx.stroke();
    ctx.beginPath(); ctx.arc(cx+9,cy-10,2.5,Math.PI,0); ctx.stroke();
    // Content smile
    ctx.strokeStyle='#e06080'; ctx.lineWidth=1;
    ctx.beginPath(); ctx.arc(cx+6,cy-6,2,0,Math.PI); ctx.stroke();
    // Nose
    ctx.fillStyle='#e06080';
    ctx.beginPath(); ctx.ellipse(cx+6,cy-7,2,1.5,0,0,Math.PI*2); ctx.fill();
    return;
  ...
  if (state === 'window_watching') {
    ...
    ctx.fillStyle='#d07820';
    // Stripes on back
    ctx.strokeStyle='#c07018'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(cx-4,cy-2); ctx.lineTo(cx-3,cy+3); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx+1,cy-3); ctx.lineTo(cx+2,cy+2); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx+5,cy-1); ctx.lineTo(cx+5,cy+4); ctx.stroke();
    // Back of head
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx,cy-8,8,7,0,0,Math.PI*2); ctx.fill();
    // Ears (twitching)
    const earTwitch = Math.sin(tick*0.08)*2;
    ctx.beginPath(); ctx.moveTo(cx-5,cy-13); ctx.lineTo(cx-4+earTwitch,cy-21); ctx.lineTo(cx-1,cy-13); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+1,cy-13); ctx.lineTo(cx+4-earTwitch,cy-21); ctx.lineTo(cx+7,cy-13); ctx.closePath(); ctx.fill();
    // Inner ears
    ctx.fillStyle='#d07050';
    ctx.beginPath(); ctx.moveTo(cx-4,cy-13); ctx.lineTo(cx-3+earTwitch,cy-19); ctx.lineTo(cx-1,cy-13); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+2,cy-13); ctx.lineTo(cx+3-earTwitch,cy-19); ctx.lineTo(cx+5,cy-13); ctx.closePath(); ctx.fill();
    // Tail wrapped
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=4;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+4); ctx.quadraticCurveTo(cx-14,cy+10,cx-6,cy+10); ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(cx-8,cy+4); ctx.quadraticCurveTo(cx-14,cy+10,cx-6,cy+10); ctx.stroke();
    return;
  }
  ...
  if (state === 'hiding') {
    ...
    ctx.quadraticCurveTo(cx+14,cy+2+tWave,cx+18,cy-2+tWave*0.5);
    ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
    ctx.beginPath();
    ctx.moveTo(cx+8,cy+6);
    ...
    // Tip of tail
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx+18,cy-2+tWave*0.5,3,2,0.5,0,Math.PI*2); ctx.fill();
    return;
  }
  ...
  if (state === 'scared') {
    ...
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=2;
    for (let i=0;i<8;i++) {
      const a = (i/8)*Math.PI*2;
      const sx2 = cx+Math.cos(a)*10, sy2 = cy+Math.sin(a)*10;
    ...
    ctx.strokeStyle='#e8902a'; ctx.lineWidth=6;
    ctx.beginPath(); ctx.moveTo(cx-8,cy); ctx.quadraticCurveTo(cx-14,cy-14,cx-10,cy-22); ctx.stroke();
    ctx.strokeStyle='#d07820'; ctx.lineWidth=3;
    ctx.beginPath(); ctx.moveTo(cx-8,cy); ctx.quadraticCurveTo(cx-14,cy-14,cx-10,cy-22); ctx.stroke();
    // Head
    ctx.fillStyle='#e8902a';
    ctx.beginPath(); ctx.ellipse(cx+4,cy-10,8,7,0,0,Math.PI*2); ctx.fill();
    // Ears perked
    ctx.beginPath(); ctx.moveTo(cx-1,cy-15); ctx.lineTo(cx+1,cy-22); ctx.lineTo(cx+5,cy-15); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+5,cy-15); ctx.lineTo(cx+8,cy-22); ctx.lineTo(cx+12,cy-15); ctx.closePath(); ctx.fill();
    // BIG scared eyes
    ctx.fillStyle='#ffffff';
    ctx.beginPath(); ctx.ellipse(cx+2,cy-11,4,4,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+9,cy-11,4,4,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#1a1020';
    ctx.beginPath(); ctx.ellipse(cx+2,cy-11,2,3.5,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+9,cy-11,2,3.5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#ffffff';
    ctx.fillRect((cx+1)|0,(cy-13)|0,1,1); ctx.fillRect((cx+8)|0,(cy-13)|0,1,1);
    // Nose
    ctx.fillStyle='#e06080';
    ctx.beginPath(); ctx.ellipse(cx+6,cy-8,2,1.5,0,0,Math.PI*2); ctx.fill();
    // Open mouth (surprised)
    ctx.fillStyle='#401020';
    ctx.beginPath(); ctx.ellipse(cx+6,cy-5,2,1.5,0,0,Math.PI*2); ctx.fill();
    ctx.restore();
    return;
  }
  ...
  ctx.quadraticCurveTo(cx-16+Math.sin(phase)*6,cy+10,cx-12+Math.cos(phase+1)*10,cy-4+Math.sin(phase)*4);
  ctx.stroke();
  ctx.strokeStyle='#d07820'; ctx.lineWidth=2;
  ctx.beginPath();
  ctx.moveTo(cx-8,cy+4);
  ...
  // Feet
  ctx.fillStyle='#d07820';
  ctx.beginPath(); ctx.ellipse(cx-4,cy+9,4,2.5,0,0,Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(cx+4,cy+9,4,2.5,0,0,Math.PI*2); ctx.fill();
  // Head
  ctx.fillStyle='#e8902a';
  ctx.beginPath(); ctx.ellipse(cx+4,cy-8,8,7,0,0,Math.PI*2); ctx.fill();
  ...
  ctx.beginPath(); ctx.moveTo(cx+5,cy-13); ctx.lineTo(cx+8,cy-20); ctx.lineTo(cx+12,cy-13); ctx.closePath(); ctx.fill();
  ctx.fillStyle='#d07050';
  ctx.beginPath(); ctx.moveTo(cx,cy-13); ctx.lineTo(cx+2,cy-18); ctx.lineTo(cx+4,cy-13); ctx.closePath(); ctx.fill();
  ctx.beginPath(); ctx.moveTo(cx+6,cy-13); ctx.lineTo(cx+8,cy-18); ctx.lineTo(cx+10,cy-13); ctx.closePath(); ctx.fill();
  // Eyes
  ...
  if (blink) {
    ...
    ctx.beginPath(); ctx.ellipse(cx+3,cy-10,2.5,3,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+9,cy-10,2.5,3,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#3a2860';
    ctx.beginPath(); ctx.ellipse(cx+3,cy-10,1.5,2.5,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+9,cy-10,1.5,2.5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#ffffff';
    ctx.fillRect((cx+2)|0,(cy-12)|0,1,1); ctx.fillRect((cx+8)|0,(cy-12)|0,1,1);
  }
  // Nose
  ...
  ctx.beginPath(); ctx.ellipse(cx+6,cy-7,2,1.5,0,0,Math.PI*2); ctx.fill();
  // Whiskers
  ctx.strokeStyle='#ffffff70'; ctx.lineWidth=0.8;
  [[cx+6,cy-7,cx-4,cy-8],[cx+6,cy-7,cx+15,cy-8],
   [cx+6,cy-6,cx-4,cy-5],[cx+6,cy-6,cx+15,cy-5]].forEach(([x1,y1,x2,y2])=>{
    ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y2); ctx.stroke();
  });
  // Tabby stripes
  ctx.strokeStyle='#c07018'; ctx.lineWidth=1.5;
  ctx.beginPath(); ctx.moveTo(cx-5,cy-2); ctx.lineTo(cx-3,cy+3); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(cx,cy-5); ctx.lineTo(cx+1,cy-1); ctx.stroke();
}

function drawMess(ctx, mx, my, tick) {
  // Poop pile with smell waves
  ctx.fillStyle='#6a3a10';
  ctx.beginPath(); ctx.ellipse(mx,my+6,7,4,0,0,Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(mx,my+2,5,4,0,0,Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(mx,my-1,3.5,3.5,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='#8a4a18';
  ctx.beginPath(); ctx.ellipse(mx-1,my+5,4,2,0,0,Math.PI*2); ctx.fill();
  // Smell lines
  ctx.strokeStyle='#60a82080'; ctx.lineWidth=1.5;
  for(let i=0;i<3;i++){
    const off=i*5, sw=Math.sin(tick*0.1+i*2.1)*3;
    ctx.beginPath();
...
function drawGoose(ctx, cx, cy, tick, state, gooseObj) {
  ...
  ctx.beginPath(); ctx.ellipse(cx,cy+11,8,2.5,0,0,Math.PI*2); ctx.fill();

  if (state === 'napping') {
    ctx.fillStyle='#f0f0f0';
    ctx.beginPath(); ctx.ellipse(cx,cy+4,10,7,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#dde0e4';
    ctx.beginPath(); ctx.ellipse(cx+2,cy+3,7,5,0.3,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#e88a20';
    ctx.beginPath(); ctx.moveTo(cx+7,cy+1); ctx.lineTo(cx+11,cy+2); ctx.lineTo(cx+7,cy+3); ctx.closePath(); ctx.fill();
    const zOff = Math.sin(tick*0.06)*2;
    ctx.fillStyle='rgba(150,180,255,0.7)'; ctx.font='bold 7px monospace';
  ...
  if (state === 'swimming') {
    ...
    ctx.beginPath(); ctx.ellipse(cx,cy+8,12,3,0,0,Math.PI*2); ctx.fill();
    ctx.strokeStyle='#4488cc50'; ctx.lineWidth=1;
    for(let i=-1;i<=1;i++){
      const rOff=Math.sin(tick*0.08+i*1.5)*3;
      ctx.beginPath(); ctx.arc(cx+i*6+rOff,cy+9,4,0,Math.PI); ctx.stroke();
    }
    ctx.fillStyle='#f0f0f0';
    ctx.beginPath(); ctx.ellipse(cx,cy+2,9,6,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#f0f0f0';
    ctx.beginPath(); ctx.ellipse(cx+4,cy-6,3.5,7,0.15,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+5,cy-13,4,3.5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#e88a20';
    ctx.beginPath(); ctx.moveTo(cx+9,cy-14); ctx.lineTo(cx+15,cy-13); ctx.lineTo(cx+9,cy-12); ctx.closePath(); ctx.fill();
    ctx.fillStyle='#111';
    ctx.beginPath(); ctx.arc(cx+6,cy-14,1.2,0,Math.PI*2); ctx.fill();
    return;
  }

  if (state === 'hiding_bush') {
    ctx.fillStyle='#2a8a40';
    ctx.beginPath(); ctx.ellipse(cx,cy+4,14,8,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#1e7030';
    ctx.beginPath(); ctx.ellipse(cx-3,cy+5,10,6,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#34a050';
    ctx.beginPath(); ctx.ellipse(cx+4,cy+3,8,5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#f0f0f0';
    ctx.beginPath(); ctx.ellipse(cx,cy-6,4,3.5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#e88a20';
    ctx.beginPath(); ctx.moveTo(cx+4,cy-7); ctx.lineTo(cx+9,cy-6); ctx.lineTo(cx+4,cy-5); ctx.closePath(); ctx.fill();
    const eyeShift = Math.sin(tick*0.1)*1.5;
    ctx.fillStyle='#111';
    ctx.beginPath(); ctx.arc(cx-1+eyeShift,cy-7,1.2,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx+2+eyeShift,cy-7,1.2,0,Math.PI*2); ctx.fill();
    return;
  }
  ...
  const peckOff = (state === 'pecking') ? (Math.sin(tick*0.3)*0.5+0.5)*8 : 0;
  const flapAngle = (state === 'flapping') ? Math.sin(tick*0.2)*0.8 : 0;
  const flapLift = (state === 'flapping') ? Math.abs(Math.sin(tick*0.2))*3 : 0;

  const bodyY = cy + 3 + lowPosture - flapLift;

  // Orange feet
  ctx.fillStyle='#e88a20';
  const footStep = (state==='waddling'||state==='stealing'||state==='chasing_agent') ? Math.sin(phase*3)*3 : 0;
  ctx.beginPath(); ctx.ellipse(cx-3+footStep,cy+11,3,1.5,0,0,Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(cx+3-footStep,cy+11,3,1.5,0,0,Math.PI*2); ctx.fill();
  ctx.strokeStyle='#d07818'; ctx.lineWidth=0.8;
  ctx.beginPath(); ctx.moveTo(cx-5+footStep,cy+11); ctx.lineTo(cx-3+footStep,cy+10); ctx.lineTo(cx-1+footStep,cy+11); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(cx+1-footStep,cy+11); ctx.lineTo(cx+3-footStep,cy+10); ctx.lineTo(cx+5-footStep,cy+11); ctx.stroke();

  // Tail feathers
  ctx.fillStyle='#e0e3e8';
  ctx.beginPath(); ctx.moveTo(cx-8,bodyY+1); ctx.lineTo(cx-14,bodyY-3); ctx.lineTo(cx-12,bodyY+3); ctx.closePath(); ctx.fill();
  ctx.beginPath(); ctx.moveTo(cx-8,bodyY+2); ctx.lineTo(cx-15,bodyY-1); ctx.lineTo(cx-13,bodyY+5); ctx.closePath(); ctx.fill();

  // Body
  ctx.fillStyle='#f0f0f0';
  ctx.beginPath(); ctx.ellipse(cx+wobble,bodyY,10,7,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='#dde0e4';
  ctx.beginPath(); ctx.ellipse(cx+wobble+1,bodyY+2,6,4,0,0,Math.PI*2); ctx.fill();
  ...
  if (state === 'flapping') {
    ...
    if (tick % 8 < 2) {
      ...
      for(let i=0;i<3;i++){
        const fx=cx+(Math.random()-0.5)*20, fy=bodyY-10+Math.random()*6;
        ctx.beginPath(); ctx.ellipse(fx,fy,2,1,Math.random()*Math.PI,0,Math.PI*2); ctx.fill();
      }
    }
  } else if (state === 'chasing_agent') {
    ctx.fillStyle='#e8eaee';
    ctx.beginPath(); ctx.ellipse(cx-6,bodyY-1,3,8,-0.3,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+6,bodyY-1,3,8,0.3,0,Math.PI*2); ctx.fill();
  } else {
    ctx.fillStyle='#e0e3e8';
    ctx.beginPath(); ctx.ellipse(cx-5,bodyY+1,3,5.5,0.15,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx+5,bodyY+1,3,5.5,-0.15,0,Math.PI*2); ctx.fill();
  }
  ...
  ctx.fillStyle='#f0f0f0';
  ctx.beginPath(); ctx.ellipse(headX,headY,4.5,3.5,0,0,Math.PI*2); ctx.fill();

  // Beak
  ctx.fillStyle='#e88a20';
  if (state === 'honking' || state === 'bothering_cat') {
    ctx.beginPath(); ctx.moveTo(headX+4,headY-2); ctx.lineTo(headX+13,headY-4); ctx.lineTo(headX+4,headY); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(headX+4,headY); ctx.lineTo(headX+13,headY+3); ctx.lineTo(headX+4,headY+2); ctx.closePath(); ctx.fill();
  } else if (state === 'stealing') {
    ctx.beginPath(); ctx.moveTo(headX+4,headY-1); ctx.lineTo(headX+11,headY); ctx.lineTo(headX+4,headY+1); ctx.closePath(); ctx.fill();
  ...
  // Eye
  ctx.fillStyle='#111';
  ctx.beginPath(); ctx.arc(headX+1,headY-1.5,1.3,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='#fff';
  ctx.beginPath(); ctx.arc(headX+0.5,headY-2,0.5,0,Math.PI*2); ctx.fill();
  ...
  if (state === 'pecking') {
    if (tick%6<2) {
      ...
      for(let i=0;i<2;i++){
        ctx.fillRect(cx+8+Math.random()*6,cy+8+Math.random()*4,2,2);
      }
    }
  }
}
...
class CatState {
  ...
  pickTarget() {
    ...
    if (fullBowls.length > 0 && Math.random() < 0.30) {
      const bowl = fullBowls[Math.floor(Math.random() * fullBowls.length)];
      this.targetTx = bowl.tx + 0.5; this.targetTy = bowl.ty + 0.5;
      this.eatTarget = bowl;
    } else {
      this.eatTarget = null;
  ...
  _findNearestDesk() {
    ...
    for (const d of DESK_DEFS) {
      const dx = d.tx - this.tx, dy = d.ty - this.ty;
  ...
  _findIdleAgent() {
    ...
    for (const [id, sp] of Object.entries(agentStates)) {
      if (sp.arrived && !sp.isWorking) {
        const dx = sp.tx - this.tx, dy = sp.ty - this.ty;
        const dist = dx*dx + dy*dy;
        if (dist < bestD) { bestD = dist; best = sp; }
      }
    }
    return best;
  }
  ...
  update(dt, tick) {
      ...
      for (const sp of Object.values(agentStates)) {
        if (sp.state === 'walking' && !sp.arrived) {
          ...
          if (Math.sqrt(dx*dx + dy*dy) < 1.5) {
            ...
            this.state = 'scared'; this.scaredTimer = 2;
            this.stateTimer = 2;
    ...
    if (this.state === 'walking') {
      ...
      if (dist < 0.25) {
        if (this.eatTarget && this.eatTarget.hasFod) {
          ...
          this.state = 'eating'; this.stateTimer = 4 + Math.random() * 5;
        } else if (this.eatTarget && !this.eatTarget.hasFod) {
          this.eatTarget = null;
          this.state = 'sitting'; this.stateTimer = 2;
        } else {
          this.state = 'sitting'; this.stateTimer = 5 + Math.random() * 10;
        }
      } else {
    ...
    if (this.state === 'scared') {
      ...
      if (this.scaredTimer <= 0) {
        this.state = 'sitting'; this.stateTimer = 3 + Math.random() * 4;
      }
    }
    ...
    if (this.state === 'hunting') {
      if (this.huntPhase === 0) {
        ...
        if (this.stateTimer < (this._huntDuration || 4) - 2) {
          ...
          this.targetTx = Math.max(2, Math.min(COLS-3, this.tx + Math.cos(angle)*2));
          this.targetTy = Math.max(2, Math.min(ROWS*0.6, this.ty + Math.sin(angle)*2));
        }
      }
      if (this.huntPhase === 1) {
        const dtx = this.targetTx - this.tx, dty = this.targetTy - this.ty;
    ...
    if (this.state === 'knocking') {
      if (this.knockPhase === 0) {
        ...
        const dist = Math.sqrt(dtx*dtx + dty*dty);
        if (dist < 0.3) {
          this.knockPhase = 1; this.stateTimer = 1.5; // pause at desk
        } else {
          const step = Math.min(1.2 * dt, dist);
          this.tx += (dtx/dist)*step; this.ty += (dty/dist)*step;
          this.flip = dtx < 0;
    ...
    if (this.state === 'zoomies') {
      ...
      this.zoomieTrails = this.zoomieTrails.filter(t => { t.life -= dt; return t.life > 0; });
    }

    // --- Purring ---
    if (this.state === 'purring') {
      if (this.purrTarget) {
        const dtx = this.purrTarget.tx - this.tx, dty = this.purrTarget.ty - this.ty;
        const dist = Math.sqrt(dtx*dtx + dty*dty);
        if (dist > 1.0) {
          const step = Math.min(1.2 * dt, dist);
          this.tx += (dtx/dist)*step; this.ty += (dty/dist)*step;
      ...
      // Heart particles
      if (tick % 20 === 0) {
        this.purrHearts.push({x: this.sx + (Math.random()-0.5)*8, y: this.sy - 15, life: 1.5});
      }
      this.purrHearts = this.purrHearts.filter(h => { h.life -= dt; h.y -= 15*dt; return h.life > 0; });
    }
    ...
    if (this.state === 'hiding') {
      if (this.hideDesk) {
        const dtx = this.hideDesk.tx + 0.5 - this.tx, dty = this.hideDesk.ty + 1 - this.ty;
        const dist = Math.sqrt(dtx*dtx + dty*dty);
        if (dist > 0.3) {
          const step = Math.min(1.2 * dt, dist);
          this.tx += (dtx/dist)*step; this.ty += (dty/dist)*step;
    ...
    if (this.state === 'eating' && this.stateTimer <= 0) {
      this.eatTarget = null;
      this.state = 'walking'; this.pickTarget(); this.stateTimer = 3 + Math.random() * 6;
    }

    // --- State transitions ---
    if (this.stateTimer <= 0 && this.state !== 'eating' && this.state !== 'scared') {
      const wasState = this.state;
      this.prevState = wasState;
      const r = Math.random();
      // Stretching only from sleeping
      if (wasState === 'sleeping' && Math.random() < 0.5) {
        this.state = 'stretching'; this.stateTimer = 3 + Math.random();
      } else if (r < 0.20) {
        this.state = 'walking'; this.pickTarget(); this.stateTimer = 4 + Math.random() * 8;
      } else if (r < 0.35) {
        this.state = 'sleeping'; this.stateTimer = 10 + Math.random() * 20;
      } else if (r < 0.45) {
        this.state = 'sitting'; this.stateTimer = 5 + Math.random() * 8;
      } else if (!this.messExists && r < 0.48) {
        this.state = 'pooping'; this.stateTimer = 4 + Math.random() * 3;
      } else if (r < 0.56) {
        this.state = 'playing'; this.playAngle = 0; this.stateTimer = 5 + Math.random() * 3;
      } else if (r < 0.66) {
        this.state = 'grooming'; this.groomPhase = 0; this.stateTimer = 6 + Math.random() * 4;
      } else if (r < 0.71) {
        this.state = 'hunting'; this.huntPhase = 0; this._huntDuration = 3 + Math.random() * 2; this.stateTimer = this._huntDuration;
      } else if (r < 0.75) {
        // Knocking — walk to nearest desk
        const desk = this._findNearestDesk();
        if (desk) {
          this.state = 'knocking'; this.knockPhase = 0; this.knockParticle = null;
          this.targetTx = desk.tx + 1; this.targetTy = desk.ty + 1;
          this.stateTimer = 6;
        } else {
          this.state = 'sitting'; this.stateTimer = 4;
        }
      } else if (r < 0.80) {
        this.state = 'zoomies'; this.zoomieTimer = 0; this.zoomieTrails = []; this.stateTimer = 4 + Math.random();
      } else if (r < 0.88) {
        // Purring — find idle agent
        const agent = this._findIdleAgent();
        if (agent) {
          this.state = 'purring'; this.purrTarget = agent; this.purrHearts = [];
          this.targetTx = agent.tx + 0.5; this.targetTy = agent.ty + 0.5;
        ...
        const desk = this._findNearestDesk();
        if (desk) {
          this.state = 'hiding'; this.hideDesk = desk; this.stateTimer = 8 + Math.random() * 4;
        } else {
          this.state = 'sitting'; this.stateTimer = 4;
        }
      } else {
        this.state = 'sitting'; this.stateTimer = 4 + Math.random() * 6;
      }
    }
    // After pooping, leave mess and walk away
    if (this.state === 'pooping' && this.stateTimer < 1 && !this.messExists) {
      this.messExists = true;
      this.messTx = this.tx; this.messTy = this.ty + 1;
      this.state = 'walking'; this.pickTarget(); this.stateTimer = 5;
    }
  }
  ...
  draw(ctx, tick) {
    ...
    ctx.save();
    if (this.flip) {
      ctx.translate(this.sx, this.sy); ctx.scale(-1,1); ctx.translate(-this.sx, -this.sy);
    }
    ...
    // Draw mess
    if (this.messExists) {
      drawMess(ctx, OX+this.messTx*T+T/2, OY+this.messTy*T+T/2, tick);
    }
  }
}
...
class GooseState {
  ...
  _moveToward(targetX, targetY, speed, dt) {
    const dtx = targetX - this.tx, dty = targetY - this.ty;
    const dist = Math.sqrt(dtx*dtx + dty*dty);
  ...
  _changeState() {
    ...
    if (Math.random() < 0.20) {
      this.state = 'honking'; this.stateTimer = 2 + Math.random() * 2;
      return;
    }
    ...
    if (Math.random() < 0.05) {
      const desk = this._findNearestDesk();
      if (desk) {
        this.state = 'stealing'; this.stolenItem = false;
        this.targetTx = desk.tx + 1; this.targetTy = desk.ty + 1;
    ...
    if (r < 0.25) {
      this.state = 'waddling'; this._pickWanderTarget(); this.stateTimer = 5 + Math.random() * 6;
    } else if (r < 0.35) {
      this.state = 'pecking'; this.stateTimer = 3 + Math.random() * 3;
    } else if (r < 0.45) {
      this.state = 'flapping'; this.stateTimer = 3 + Math.random() * 2;
    } else if (r < 0.55) {
      const agent = this._findRandomAgent();
      if (agent) {
        this.state = 'chasing_agent'; this.chaseTarget = agent;
        this.chaseTimer = 8 + Math.random() * 2;
        this.stateTimer = this.chaseTimer;
      } else {
        this.state = 'waddling'; this._pickWanderTarget(); this.stateTimer = 4;
      }
    } else if (r < 0.62) {
      const plant = this._findPlantSpot();
      if (plant) {
        this.state = 'hiding_bush'; this.plantSpot = plant; this.stateTimer = 6 + Math.random() * 4;
      } else {
        this.state = 'waddling'; this._pickWanderTarget(); this.stateTimer = 4;
      }
    } else if (r < 0.70) {
      this.state = 'bothering_cat'; this.stateTimer = 5 + Math.random() * 3;
    } else if (r < 0.78) {
      const aq = this._findAquariumSpot();
      if (aq) {
        this.state = 'swimming'; this._swimTarget = aq; this.stateTimer = 8 + Math.random() * 5;
      } else {
        this.state = 'pecking'; this.stateTimer = 3;
      }
    } else if (r < 0.88) {
      this.state = 'napping'; this.stateTimer = 10 + Math.random() * 10;
    } else {
      this.state = 'honking'; this.stateTimer = 2 + Math.random() * 2;
    }
  }
...
function drawLeftPanel(ctx, tick) {
  ...
  for (const [id, sp] of entries) {
    ...
    ctx.fillStyle = dotC;
    ctx.beginPath(); ctx.arc(13, y-3, 4, 0, Math.PI*2); ctx.fill();
    if (isWork) {
      ctx.fillStyle = dotC + '40';
      ctx.beginPath(); ctx.arc(13, y-3, 7, 0, Math.PI*2); ctx.fill();
    }
...
function drawRightPanel(ctx, tick) {
  ...
  // Tasks
  const doneTasks = (typeof myTasks !== 'undefined') ? myTasks.filter(t=>t.status==='done').length : 0;
  const todoTasks = (typeof myTasks !== 'undefined') ? myTasks.filter(t=>t.status==='todo').length : 0;
  ctx.fillStyle = '#2a2c4e';
  ...
  ctx.fillStyle = '#565f89'; ctx.font = '9px monospace'; ctx.textAlign='left';
  ctx.fillText('CAT', panelX+8, 270);
  const _catMoods = {sleeping:'😴 asleep',eating:'🍽 eating',playing:'🎾 playing',grooming:'🧼 grooming',stretching:'🙆 stretch',hunting:'🎯 hunting',knocking:'😈 knocking',zoomies:'💨 zoomies!',purring:'💕 purring',window_watching:'🪟 watching',hiding:'🙈 hiding',scared:'😱 scared!',pooping:'💩 pooping'};
  ...
  const _gooseMoods = {waddling:'🦆 waddling',honking:'📢 HONK!',stealing:'🏃 stealing!',chasing_agent:'😤 chasing',swimming:'🏊 swimming',flapping:'🪽 flapping',pecking:'🐦 pecking',hiding_bush:'🌿 hiding',bothering_cat:'😈 annoying',napping:'😴 napping'};
  ...
  ctx.fillStyle = '#565f89'; ctx.font = '9px monospace';
  ...
  if ((tick >> 4) & 1) {
    ctx.fillStyle = '#9ece6a';
    ctx.beginPath(); ctx.arc(panelX + W - 10, 14, 4, 0, Math.PI*2); ctx.fill();
  }
...
function loop(now) {
  ...
  for (const id of Object.keys(agentStates)) {
    if (!agentsData[id]) {
      ...
      sndRemove();
      doorAnim.target = 1; doorAnim.timer = 1.8; // door opens on departure
  ...
  for (const [id, a] of Object.entries(agentsData)) {
    if (!agentStates[id]) {
      ...
      sndSpawn();
      doorAnim.target = 1; doorAnim.timer = 1.8; // door opens on arrival
    }
  }
  ...
  if (generateLayout(Math.max(1, n))) {
    ...
    for (const sp of Object.values(agentStates)) { sp.slotIdx=-1; sp.arrived=false; sp.activityDur=0; }
  ...
  for (const [id,sp] of Object.entries(agentStates)) sp.update(agentsData[id],deskSet,dt,globalTick);
    ...
    for (let i = 0; i < list.length; i++) {
      for (let j = i + 1; j < list.length; j++) {
        ...
        if (!a.arrived && !b.arrived) {
          ...
          a.tx += nx * 0.5; a.ty += ny * 0.5;
          b.tx -= nx * 0.5; b.ty -= ny * 0.5;
        } else if (!a.arrived) {
  ...
  // ── High-five when both agents working at nearby desks ─────────
  {
    ...
    for (let i = 0; i < list.length; i++) {
      for (let j = i + 1; j < list.length; j++) {
        ...
        if (!da || !db) continue;
        const dx = da.tx - db.tx, dy = da.ty - db.ty;
        if (dx*dx + dy*dy > HIGHFIVE_DIST2) continue;
        // Trigger high-five
        a.highFiveTimer = 2.2;
        b.highFiveTimer = 2.2;
        // Burst particles at midpoint between the two agents
        const mx = (a.sx + b.sx) / 2, my = (a.sy + b.sy) / 2 - 20;
        PS.emit(mx, my, 10, {
          vx: 0, vy: -40, spread: 80, gravity: 120,
          life: 0.7, size: 3,
  ...
  // ── Ping pong ball physics ──────────────────────────────────────
  {
    ...
    const bothHere = ppPair && ppPair.every(idx => {
    ...
    if (ppBall.active) {
      ...
      if (ppBall.t >= 1) { ppBall.t = 1; ppBall.dir = -1; }
      if (ppBall.t <= 0) { ppBall.t = 0; ppBall.dir =  1; }
    }
  }
  ...
  if (!cat.messExists) {
    if (cat.cleaningAgentId && agentStates[cat.cleaningAgentId]) {
      ...
      sp.isCleaning = false; sp._cleanTimer = 0;
      sp.slotIdx = -1; sp.activityDur = 0; sp.arrived = false;
    }
    cat.cleaningAgentId = null; cat.messAssignTimer = 0;
  } else {
    ...
    if (cat.cleaningAgentId === null) {
      ...
      if (cat.messAssignTimer >= 3) {
        ...
        for (const [aid, sp] of Object.entries(agentStates)) {
          ...
          const dx = sp.tx - cat.messTx, dy = sp.ty - cat.messTy;
          const d = Math.sqrt(dx*dx + dy*dy);
          if (d < nearestDist) { nearestDist = d; nearest = aid; }
        }
        if (nearest) {
          const sp = agentStates[nearest];
          if (sp.slotIdx >= 0 && idleOccupied[sp.slotIdx] === nearest)
            delete idleOccupied[sp.slotIdx];
          sp.slotIdx = -1; sp.activityDur = 0;
          sp.isCleaning = true; sp._cleanTimer = 0; sp.arrived = false;
          cat.cleaningAgentId = nearest; cat.messAssignTimer = 0;
        } else {
    ...
    if (cat.cleaningAgentId && agentStates[cat.cleaningAgentId]) {
      const sp = agentStates[cat.cleaningAgentId];
      const dx = cat.messTx - sp.tx, dy = cat.messTy - sp.ty;
      const dist = Math.sqrt(dx*dx + dy*dy);
      if (dist > 1.0) {
        if (sp.state !== 'walking') sp.setAnim('walking');
        const step = Math.min(3.5 * dt, dist);
        sp.tx += (dx/dist)*step; sp.ty += (dy/dist)*step;
        sp.flip = dx < 0;
      } else {
  ...
  for (let bi = 0; bi < CAT_BOWLS.length; bi++) {
    ...
    if (!sp || sp.isWorking) { delete bowlRefills[bi]; continue; } // agent left
    const bdx = sp.tx - (bowl.tx + 0.5), bdy = sp.ty - (bowl.ty + 0.5);
    ...
    if (distToBowl > 1.2) {
      ...
      if (refill.timer >= 3) {
        ...
        sp.arrived = false; sp.activityDur = 0;
      }
    }
  }
  ...
  for (const [id, sp] of Object.entries(agentStates)) {
    ...
    const dx = sp.tx - cat.tx, dy = sp.ty - cat.ty;
    const distToCat = Math.sqrt(dx*dx + dy*dy);
    // Pet the cat if nearby and cat is sitting/sleeping
    if (distToCat < 2.5 && (cat.state === 'sitting' || cat.state === 'sleeping')
        && sp.state !== 'petting_cat' && sp.state !== 'cleaning') {
  ...
  if (trashLevel >= 8 && trashAgentId === null) {
    ...
    const trashTx = KITCHEN_WALL_COL - 2 + 0.5, trashTy = 3 + 0.5;
    let nearest = null, nearestDist = Infinity;
    for (const [aid, sp] of Object.entries(agentStates)) {
      if (sp.isWorking || sp.isCleaning) continue;
      const dx = sp.tx - trashTx, dy = sp.ty - trashTy;
      const d = Math.sqrt(dx*dx + dy*dy);
      if (d < nearestDist) { nearestDist = d; nearest = aid; }
    }
    if (nearest) { trashAgentId = nearest; agentStates[nearest].isCleaning = true; }
  }
  if (trashAgentId && agentStates[trashAgentId]) {
    const sp = agentStates[trashAgentId];
    if (sp.isWorking) { sp.isCleaning = false; trashAgentId = null; }
    else {
      const trashTx = KITCHEN_WALL_COL - 2 + 0.5, trashTy = 3 + 0.5;
      const dx = trashTx - sp.tx, dy = trashTy - sp.ty;
      const dist = Math.sqrt(dx*dx + dy*dy);
      if (dist > 1.2) {
  ...
  if (ACT_ZONE_Y > 0) {
    const anyGaming = Object.values(agentStates).some(s => s.activityAnim === 'gaming' && s.arrived);
    const [_tvATx2,_tvATy2] = getAdminPos('tv', 10, ACT_ZONE_Y+0.3);
    const [tvx, tvy] = ts(_tvATx2, _tvATy2);
    const sx = tvx + 6, sy = tvy + 6, sw = T*4 - 14, sh = T*2.5 - 12;
    if (anyGaming) {
      ...
      // Sky gradient
      ctx.fillStyle = '#0d1a3a'; ctx.fillRect(sx, sy, sw, sh);
      // Scrolling ground
      ctx.fillStyle = '#1a3a10'; ctx.fillRect(sx, sy + sh - 10, sw, 10);
      // Platforms (scrolling left)
      const scroll = (globalTick * 1.2) % (sw + 30);
      ctx.fillStyle = '#2a5a20';
      [[sw - scroll, sh - 20, 24, 6],[sw - scroll + 50, sh - 32, 20, 6],[sw - scroll + 90, sh - 18, 18, 6]].forEach(([px,py,pw,ph]) => {
        if (px > -pw && px < sw) ctx.fillRect(sx + px, sy + py, pw, ph);
      });
      // Player character running
      const runFrame = gf;
      const pcy = sy + sh - 22;
      ctx.fillStyle = '#f7768e'; ctx.fillRect(sx + 18, pcy, 8, 10); // body
      ctx.fillStyle = '#f5c2a0'; ctx.fillRect(sx + 19, pcy - 7, 6, 7); // head
      ctx.fillStyle = '#3d59a1'; ctx.fillRect(sx + 17 + (runFrame%2), pcy + 7, 4, 5); // leg L
      ctx.fillStyle = '#3d59a1'; ctx.fillRect(sx + 22 - (runFrame%2), pcy + 7, 4, 5); // leg R
      // Enemy
      const ex = sx + sw - 30 - (globalTick % 20 < 10 ? 1 : 0);
      ctx.fillStyle = '#e0af68'; ctx.fillRect(ex, pcy + 2, 10, 8);
      ctx.fillStyle = '#f7768e'; ctx.fillRect(ex + 2, pcy - 2, 6, 6);
      // Score (incrementing)
      ctx.fillStyle = '#9ece6a'; ctx.font = "4px 'Press Start 2P',monospace";
      ctx.textAlign = 'left';
      ...
      ctx.save();
      ctx.beginPath(); ctx.rect(sx, sy, sw, sh); ctx.clip();
      ctx.fillStyle = '#050510'; ctx.fillRect(sx, sy, sw, sh);
...
let wsActive=false;
function connect() {
  if (wsActive) return; wsActive=true;
  ...
  ws.onmessage = ({data}) => {
    ...
    if (msg.type==='public_url') {
      ...
      if (msg.url) { el.href=msg.url; el.textContent='🌐 '+msg.url.replace('https://',''); el.className=''; }
      else         { el.href='#'; el.textContent='🔒 local'; el.className='loading'; }
    }
  };
  ws.onclose = () => { wsActive=false; led.className='led off'; lbl.textContent='polling'; setTimeout(connect,2000); startPolling(); };
  ws.onerror = () => ws.close();
}
...
function startPolling() {
  ...
  pollInterval = setInterval(async () => {
    if (wsActive) { clearInterval(pollInterval); pollInterval = null; return; }
    try {
      ...
      if (data.agents) {
        ...
        for (const id of Object.keys(agentsData)) {
          if (!newIds.has(id)) { delete agentsData[id]; document.getElementById(`c-${id}`)?.remove(); }
        }
        for (const a of data.agents) { agentsData[a.id] = a; renderCard(a); }
      }
      if (data.myTasks) { myTasks = data.myTasks; renderTasks(); }
      updateUI();
      const led = document.getElementById('led'), lbl = document.getElementById('conn-label');
      led.className = 'led'; lbl.textContent = 'polling';
    } catch {}
  }, 2000);
}
...
function buildAdminObjects() {
  ...
  if (ACT_ZONE_Y > 0) {
    ...
    adminObjects.push({id:'nap_pod', label:'😴 Nap Pod', tx:22, ty:ACT_ZONE_Y+14, w:2.5, h:1.5});
