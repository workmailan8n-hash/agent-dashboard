...
function buildBackground() {
  ...
  if (ACT_ZONE_Y > 0) {
    ...
    const wbW = T*3.5, wbH = T*2;
    // Wall mount brackets
    fillR(ctx, wbx-2, wby+6, 4, wbH-12, '#404050');
    fillR(ctx, wbx+wbW-2, wby+6, 4, wbH-12, '#404050');
    // Whiteboard frame (dark border)
    ctx.save(); ctx.shadowColor='#00000060'; ctx.shadowBlur=6; ctx.shadowOffsetY=3;
    fillR(ctx, wbx, wby, wbW, wbH, '#2a2840'); ctx.restore();
    ...
    const bx2 = wbx+6, by2 = wby+8;
    // Boxes (system components)
    ctx.strokeStyle='#4060a0'; ctx.lineWidth=1.5;
    ...
    ctx.beginPath(); ctx.moveTo(bx2+20,by2+7); ctx.lineTo(bx2+24,by2+7); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(bx2+11,by2+12); ctx.lineTo(bx2+21,by2+20); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(bx2+33,by2+12); ctx.lineTo(bx2+30,by2+20); ctx.stroke();
    // Labels in boxes
    ctx.fillStyle='#303060'; ctx.font='3px monospace';
    ctx.fillText('API', bx2+6, by2+9);
    ctx.fillText('DB', bx2+30, by2+9);
    ctx.fillText('UI', bx2+17, by2+27);
    // Scribbled notes (colourful)
    ctx.fillStyle='#9ece6a'; ctx.font='3px monospace';
    ...
    // Label above board
    ctx.fillStyle='#606070'; ctx.font='bold 7px monospace';
  ...
  if (ROWS > 20) {
    ...
    const stepW = T*0.9, stepH = 8;
    const steps = 5;
    // Wall backplate
    fillR(ctx, stx, sty-T*0.5, T*5, T*2+8, '#252344');
    fillR(ctx, stx, sty-T*0.5, T*5, 3, '#3a3860'); // top edge
    // Stair handrails (left and right)
    ctx.strokeStyle='#808098'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(stx+2, sty+T*1.5); ctx.lineTo(stx+T*5-2, sty-T*0.3); ctx.stroke();
    ctx.strokeStyle='#606078'; ctx.lineWidth=1;
    ctx.beginPath(); ctx.moveTo(stx+2, sty+T*1.5+3); ctx.lineTo(stx+T*5-2, sty-T*0.3+3); ctx.stroke();
    ...
    ctx.fillStyle='#e0af68'; ctx.font='bold 8px monospace';
    ctx.textAlign='center';
    ctx.fillText('2F ↑', stx+T*2.5, sty-T*0.7);
    ctx.textAlign='left';
    // Arrow pointing up on wall
    ctx.strokeStyle='#e0af6880'; ctx.lineWidth=1.5;
    ctx.beginPath();
    ...
    ctx.beginPath(); ctx.ellipse(stx+T*2.5, sty+T*1.5+6, T*2, 4, 0, 0, Math.PI*2); ctx.fill();
  }
