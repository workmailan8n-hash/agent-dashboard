...
function handlePostSync(req, res, agents, agentTasks, broadcast) {
  let body = '',
    size = 0;
  req.on('data', (d) => {
    size += d.length;
...
function handleGetState(req, res, agents, agentTasks) {
  res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
  res.end(JSON.stringify({ agents: Object.values(agents), tasks: agentTasks }));
}
